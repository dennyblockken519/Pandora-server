...
public class Index {
     ...
     */
    public mgi.utilities.ByteBuffer get(int group) {
        synchronized (lock) {
            try {
                if ((long) group * IDX_BLOCK_LEN + IDX_BLOCK_LEN > indexChannel.size()) {
                    return null;
                }

                tempBuffer.position(0).limit(IDX_BLOCK_LEN);
                indexChannel.read(tempBuffer, (long) group * IDX_BLOCK_LEN);
                tempBuffer.flip();
                ...
                mgi.utilities.ByteBuffer groupBuffer = new mgi.utilities.ByteBuffer(size);
                int remaining = size;
                int chunk = 0;
    ...
    private boolean put(int group, ByteBuffer data, int size, boolean exists) {
        synchronized (lock) {
            try {
                ...
                tempBuffer.flip();
                indexChannel.write(tempBuffer, (long) group * IDX_BLOCK_LEN);
