...
public class Group {
    ...
    public Group(int groupID, int name, int version, File[] files) {
        for (int i = 0; i < files.length; i++) if (!files[i].isLoaded()) throw new RuntimeException("Nonloaded file.");
        this.id = groupID;
        this.name = name;
        this.version = version;
        this.files = files;
        for (int i = 0; i < files.length; i++) if (files[i].getID() == -1) files[i].setID(getFreeFileID());
    ...
    public Group(int groupID, int name, int version, int crc32, byte[] digest, int[] filesIDS, int[] filesNames) {
        ...
        for (int i = 0; i < filesIDS.length; i++) files[i] = new File(filesIDS[i], filesNames[i]);
    }
    ...
    public void unload() {
        ...
        packedFiles = null;
        for (int i = 0; i < files.length; i++) files[i].unload();
    }
    ...
    private void unpack(ByteBuffer packed) {
        ...
        for (int sectorID = 0; sectorID < sectorsCount; sectorID++)
            for (int i = 0, length = 0; i < fileCount(); i++) lengths[i] += (length += packed.readInt());
    ...
    private ByteBuffer pack() {
        ...
        for (int i = 0; i < files.length; i++) allocLength += files[i].getData().getBuffer().length;
    ...
    public int getFreeFileID() {
        ...
        int highest = -1;
        for (int i = 0; i < files.length; i++) if (files[i].getID() > highest) highest = files[i].getID();
        return highest + 1;
    }
    ...
    public File findFileByID(int id) {
        ...
        for (int i = 0; i < files.length; i++) if (files[i].getID() == id) return files[i];
        return null;
    }
    ...
    public File findFileByName(int name) {
        ...
        for (int i = 0; i < files.length; i++) if (files[i].getName() == name) return files[i];
        return null;
    }
    ...
    private boolean needsRepack() {
        ...
        for (int i = 0; i < files.length; i++) if (files[i].isDataChanged()) return true;
        return false;
    }
    ...
    private void markAsNotNeedRepack() {
        needsRepack = false;
        for (int i = 0; i < files.length; i++) files[i].markDataAsNotChanged();
    }
    ...
    public boolean isIndexChanged() {
        ...
        for (int i = 0; i < files.length; i++) if (files[i].isIndexInfoChanged()) return true;
        return false;
    }
    ...
    public void markIndexAsNotChanged() {
        indexChanged = false;
        for (int i = 0; i < files.length; i++) files[i].markIndexAsNotChanged();
    }
