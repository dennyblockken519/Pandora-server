...
public class EasterMapPacker {
    ...
    public final void packAll() throws IOException {
        ...
        packMap(8772, Files.readAllBytes(Paths.get("assets/easter/map/m_9811.dat")), MapUtils.inject(MapUtils.encode(scourgeObjs).getBuffer(), o -> false));
    }
    ...
    private static void packMusic() throws IOException {
        Game.getCacheMgi().getArchive(ArchiveType.MUSIC).addGroup(new Group(2502, new mgi.tools.jagcached.cache.File(new ByteBuffer(IOUtils.toByteArray(new FileInputStream(new File("assets/easter/music/Lazy Wabbit.dat")))))));
            ...
            if (ArrayUtils.contains(objectsToTransform, obj.getId())) {
                if (obj.getTransmogrifiedIds() != null) {
                    ...
                    for (final Integer transform : transforms) {
                        final ObjectDefinitions def = Utils.findMatching(objSet, t -> t.getId() == transform);
    ...
    public void packModels() throws IOException {
        ...
        for (final Integer originalId : list) {
            java.io.File file = new File("assets/easter/models/" + originalId + ".dat");
            if (!file.exists()) {
    ...
    public void packGraphics() throws IOException {
        ...
        for (final Int2ObjectMap.Entry<java.io.File> entry : sortedMap.int2ObjectEntrySet()) {
            final java.io.File file = entry.getValue();
            final GraphicsDefinitions def = new GraphicsDefinitions(ChristmasMapPacker.graphicsId++, new ByteBuffer(FileUtils.readFileToByteArray(file)));
    ...
    private void packAnimationBase(@NotNull final AnimationBase base, final int[] anims) throws IOException {
        for (int anim : anims) {
            final java.io.File file = new File("assets/easter/animation definitions/" + anim + ".dat");
    ...
    private static final int[] objectsToTransform = new int[]{46403, 46404, 46407, 46411, 46412, 46413, 46414, 46415, 46334, 46333};
    ...
    public void packNPCs() throws IOException {
        ...
        //7427, 7443
        //Lets make an exception for a known problematic npc of id 15197
        final NPCDefinitions matching = Utils.findMatching(objSet, o -> o.getId() == 15197);
        final NPCDefinitions def = Utils.findMatching(objSet, o -> o.getId() == 15213);
