...
public class ByteBuffer {
    ...
    private static final char[] unicodeTable = {'€', '\000', '‚', 'ƒ', '„', '…', '†', '‡', 'ˆ', '‰', 'Š', '‹', 'Œ', '\000', 'Ž', '\000', '\000', '‘', '’', '“', '”', '•', '–', '—', '˜', '™', 'š', '›', 'œ', '\000', 'ž', 'Ÿ'};
    ...
    public byte[] toArray(int offset, int length) {
        ...
        if (length >= 0) System.arraycopy(this.buffer, offset + 0, bf, 0, length);
        return bf;
    }
    ...
    public final int readSignedMedInt() {
        this.position += 3;
        int value = ((this.buffer[this.position - 1] & 255) + (this.buffer[this.position - 2] << 8 & 65280) + (this.buffer[this.position - 3] << 16 & 16711680));
        if (value > 8388607) {
            value -= 16777216;
        }
        return value;
    ...
    public Int2ObjectOpenHashMap<Object> readParameters() {
        final int length = readUnsignedByte();
        final Int2ObjectOpenHashMap<Object> parameters = new Int2ObjectOpenHashMap<>(length);
    ...
    public void writeParameters(final Int2ObjectOpenHashMap<Object> parameters) {
