...
abstract class AbstractEnum<V> implements Enum<V> {
     ...
     */
    AbstractEnum(final int id, final char keyType, final char valType, final V defaultValue, final Map<Integer, ?> map) {
        this.id = id;
     ...
     */
    abstract boolean populate(final Map<Integer, ?> map, final Map<Integer, V> populatedMap, final Map<V, Integer> reversePopulatedMap);
