...
public final class IntEnum extends AbstractEnum<Integer> {
    ...
    public IntEnum(final int id, final char keyType, final char valType, final int defaultValue, final Map<Integer, ?> map) {
        ...
        if (size == 0) {
            intMap = reverseIntMap = null;
            containsOverlappingValues = false;
            return;
        }
        Int2IntMap intMap = new Int2IntOpenHashMap(size);
        Int2IntMap reverseIntMap = new Int2IntOpenHashMap(size);
    ...
    boolean containsOverlappingValues() {
        return this.containsOverlappingValues;
    }
}
