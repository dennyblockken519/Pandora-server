...
public final class NPCExtractor implements Extractor {
    ...
    public void extract() {
        try {
            final BufferedWriter writer = new BufferedWriter(new FileWriter(new File("info/#" + Constants.REVISION + " npc-defs.txt")));
            //final int len = CacheUtil.getStore().getFilesSystem(2).findFolderByID(9).filesCount();//Cache.STORE.getIndexes()[2].getLastFileId(9);
            final int len = Utils.getIndiceSize(Indice.NPC_DEFINITIONS);
            for (int i = 0; i < len; i++) {
