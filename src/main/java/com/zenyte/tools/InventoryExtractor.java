...
public final class InventoryExtractor implements Extractor {
    ...
    public void extract() {
        try {
            final BufferedWriter writer = new BufferedWriter(new FileWriter(new File("info/#" + Constants.REVISION + " inventory-sizes.txt")));
            //final int len = CacheUtil.getStore().getFilesSystem(2).findFolderByID(5).filesCount();//Cache.STORE.getIndexes()[2].getLastFileId(5);
            final int len = Utils.getIndiceSize(Indice.INVENTORY_SIZE_DEFINITIONS);
            for (int i = 0; i < len; i++) {
