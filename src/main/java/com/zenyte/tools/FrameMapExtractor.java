...
public class FrameMapExtractor implements Extractor {
    ...
    public void extract() {
        try {
            final BufferedWriter writer = new BufferedWriter(new FileWriter(new File("info/#" + Constants.REVISION + " frame map.txt")));
            ...
            map.forEach((i, list) -> {
                try {
                    ...
                    for (int a = 0; a < Utils.getIndiceSize(Indice.GRAPHICS_DEFINITIONS); a++) {
                        final GraphicsDefinitions definitions = GraphicsDefinitions.get(a);
                        if (definitions == null) {
                            continue;
                        }
                    ...
                    if (!linkedGFXs.isEmpty()) {
                        ...
                        for (final int id : linkedGFXs) {
                            final GraphicsDefinitions graphicsDefinitions = GraphicsDefinitions.get(id);
