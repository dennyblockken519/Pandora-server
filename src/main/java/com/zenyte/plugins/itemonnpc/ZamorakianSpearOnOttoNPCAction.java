...
public class ZamorakianSpearOnOttoNPCAction implements ItemOnNPCAction {
    ...
    public void handleItemOnNPCAction(final Player player, final Item item, final int slot, final NPC npc) {
        final boolean canWield = player.getSkills().getLevel(Skills.ATTACK) >= 70;
        player.getDialogueManager().start(new Dialogue(player, npc) {
            @Override
            public void buildDialogue() {
                npc("Yes, I can convert a Zamorakian spear into a hasta.<br>The spirits require me to request " + Utils.format(COST.getAmount()) + " coins from<br>you for this service.");
                if (!canWield) {
