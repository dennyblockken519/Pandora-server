...
public class InfernalToolInfusion implements PairedItemOnItemPlugin {
    ...
    private enum InfernalTool {
        AXE(Skills.WOODCUTTING, Skills.FIREMAKING, 61, 85, new int[]{ItemId.DRAGON_AXE, ItemId.INFERNAL_AXE_UNCHARGED}, ItemId.INFERNAL_AXE),
        PICKAXE(Skills.MINING, Skills.SMITHING, 61, 85, new int[]{ItemId.DRAGON_PICKAXE, ItemId.INFERNAL_PICKAXE_UNCHARGED}, ItemId.INFERNAL_PICKAXE),
        HARPOON(Skills.FISHING, Skills.COOKING, 75, 85, new int[]{ItemId.DRAGON_HARPOON, ItemId.INFERNAL_HARPOON_UNCHARGED}, ItemId.INFERNAL_HARPOON);
        ...
        InfernalTool(final int primarySkill, final int secondarySkill, final int primaryRequirement, final int secondaryRequirement, final int[] usableWeaponry, final int product) {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        final Item consumedItem = (from.getId() == SMOULDERING_STONE || (Utils.findMatching(InfernalTool.values, value -> (value.usableWeaponry[1] == to.getId() || value.product == to.getId())) != null)) ? from : to;
        ...
        final InfernalToolInfusion.InfernalTool constant = Objects.requireNonNull(Utils.findMatching(InfernalTool.values, value -> value.product == tool.getId() || ArrayUtils.contains(value.usableWeaponry, tool.getId())));
        ...
        if (skills.getLevelForXp(constant.primarySkill) < constant.primaryRequirement || skills.getLevelForXp(constant.secondarySkill) < constant.secondaryRequirement) {
            player.getDialogueManager().start(new DoubleItemChat(player, consumedItem, tool, "You need a " + Skills.SKILLS[constant.primarySkill] + " level of at least " + constant.primaryRequirement + " and a " + Skills.SKILLS[constant.secondarySkill] + " level of at least " + constant.secondaryRequirement + " to infuse the two together."));
            return;
        }
        player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                ...
                item(10, new Item(constant.product), "You infuse the smouldering stone into the " + productName.replace("Infernal ", Strings.EMPTY) + " to make an " + productName.toLowerCase() + ".");
            }
        });
    }
