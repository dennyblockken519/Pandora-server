...
public class RangingGuildDoor implements ObjectAction {
    ...
    public void handle(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        player.setRouteEvent(new TileEvent(player,
                new TileStrategy(!player.inArea("Ranging Guild") ? new Location(2657, 3439, 0) : new Location(2659, 3437, 0)),
                getRunnable(player, object, name, optionId, option), getDelay()));
    }

    @Override
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        if (player.getSkills().getLevelForXp(Skills.RANGED) < 40) {
