        ...
        switch (option) {
            case "Climb":
                player.getDialogueManager().start(new OptionDialogue(player, "Which way do you want to go?",
                        ...
                        new Runnable[]{
                                () -> climb(player, object, new Location(player.getX(), player.getY(), object.getPlane() + 1), CLIMB_UP),
                                () -> climb(player, object, new Location(player.getX(), player.getY(), object.getPlane() - 1), CLIMB_DOWN),
                                null
                        }));
