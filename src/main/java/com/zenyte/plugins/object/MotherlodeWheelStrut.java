 ...
 */
public class MotherlodeWheelStrut implements ObjectAction {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        ...
        if (WHEELS.get(top) == null || MotherlodeArea.WATER_WHEELS.get(top)) return;
        if (!WHEELS.get(top)) {
            ...
            WorldTasksManager.schedule(() -> {
                ...
                if (!World.containsObjectWithId(HOPPER_BUBBLES, 2018)) World.spawnObject(HOPPER_BUBBLES);
                if (!World.containsObjectWithId(TOP_CURVE_BUBBLES, 2018)) World.spawnObject(TOP_CURVE_BUBBLES);
                if (!World.containsObjectWithId(BOTTOM_BUBBLES_EAST, 2018)) World.spawnObject(BOTTOM_BUBBLES_EAST);
                if (!World.containsObjectWithId(BOTTOM_BUBBLES_WEST, 2018)) World.spawnObject(BOTTOM_BUBBLES_WEST);
                player.unlock();
                player.getSkills().addXp(Skills.SMITHING, (player.getSkills().getLevelForXp(Skills.SMITHING) * 1.5));
