...
public final class WildernessObeliskOA implements ObjectAction {
    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        if (option.equals("Set Destination")) {
            if (!DiaryReward.WILDERNESS_SWORD3.eligibleFor(player)) {
        ...
        if (option.equals("Teleport to Destination")) {
            ...
            final ArrayList<Location> list = new ArrayList<>(Arrays.asList(CENTER_OBELISK_TILES));
    ...
    public Object[] getObjects() {
        return new Object[]{14826, 14827, 14828, 14829, 14830, 14831};
    }
}
