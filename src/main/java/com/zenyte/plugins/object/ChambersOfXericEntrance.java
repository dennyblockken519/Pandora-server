...
public final class ChambersOfXericEntrance implements ObjectAction {
    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        if (!Constants.CHAMBERS_OF_XERIC) {
        ...
        if (player.getAttributes().get("has entered chambers of xeric") == null) {
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    plain("WARNING!<br>" +
                            "Items left in a dungeon " + Colour.RS_RED.wrap("cannot") + " be retrieved.<br>" +
                            "Participants " + Colour.RS_RED.wrap("may be ejected") + " at any time.<br>" +
