...
public class Zahur extends NPCPlugin {
    ...
    public static void crushSecondaries(@NotNull final Player player, @NotNull final NPC npc) {
        player.getDialogueManager().finish();
        final Inventory inventory = player.getInventory();
        final long cost = calculateSecondariesCost(player, npc);
        final boolean free = player.getMemberRank().eligibleTo(MemberRank.ONYX_MEMBER);
        if (cost == -1) {
            return;
        }
        player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                options("Crush the secondaries for " + (free ? "free" : Utils.format(cost)) + "?", new DialogueOption("Yes, crush them.", () -> {
                    player.getDialogueManager().finish();
                    final long cost = calculateSecondariesCost(player, npc);
                    if (cost == -1) {
                        return;
                    ...
                    for (int i = 0; i < 28; i++) {
                        ...
                        final Zahur.HerbloreSecondary secondary = Utils.findMatching(HerbloreSecondary.values, sec -> (sec.rawItem == id && sec.processedItem != -1) || (sec.rawNotedItem == id && (sec == HerbloreSecondary.LAVA_SCALE || sec.processedNotedItem != -1)));
                        if (secondary != null) {
    ...
    private static long calculateSecondariesCost(@NotNull final Player player, @NotNull final NPC npc) {
        final Inventory inventory = player.getInventory();
        int amount = 0;
        for (int i = 0; i < 28; i++) {
            final Item item = inventory.getItem(i);
    ...
    private void makeUnfinishedPotions(final Player player, final NPC npc) {
        ...
        for (final Int2ObjectMap.Entry<Item> entry : player.getInventory().getContainer().getItems().int2ObjectEntrySet()) {
            ...
            final Herb herb = Herb.get(item.getId(), DiaryReward.DESERT_AMULET3.eligibleFor(player) || SkillcapePerk.HERBLORE.isEffective(player));
            if (herb == null) {
                continue;
            }
            herbs.put(herb, item);
        ...
        int amount = player.getMemberRank().eligibleTo(MemberRank.ZENYTE_MEMBER) ? Integer.MAX_VALUE : price == 0 ? vialsOfWater : Math.min(coins / price, vialsOfWater);
    ...
    private boolean decant(final Player player, final NPC npc, final int selectedDose) {
        final HashMap<Potion, PotionResult> potions = new HashMap<>();
        for (final Int2ObjectMap.Entry<Item> entry : player.getInventory().getContainer().getItems().int2ObjectEntrySet()) {
            if (entry == null) {
                continue;
            }
            final Item item = entry.getValue();
            final Potion potion = Potion.get(item.getId());
            if (potion == null) {
                continue;
            }
            if (potions.containsKey(potion)) {
                ...
                final PotionResult result = new PotionResult(potion, item);
        ...
        for (final Map.Entry<Potion, PotionResult> entry : potions.entrySet()) {
            if (entry == null) {
                continue;
            }
