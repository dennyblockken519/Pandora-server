...
public class Ilfeen extends NPCPlugin {
        ...
        bind("Talk-to", (player, npc) -> {
            player.getDialogueManager().start(new Dialogue(player, npc) {
                ...
                public void buildDialogue() {
                    ...
                    npc("What would you like me to enchant it to? It'll cost you " + (Utils.format(Math.max(180000, 900000 - (player.getNumericAttribute("crystal recharge count").intValue() * 180000)))) + " coins and a crystal seed to do this.");
                    options(TITLE, new DialogueOption("Crystal bow", () -> enchant(player, 4212)), new DialogueOption("Crystal shield", () -> enchant(player, 4224)), new DialogueOption("Crystal halberd", () -> enchant(player, 13091)));
                }
            });
        });
    }
    ...
    private final void enchant(final Player player, final int itemId) {
        ...
        if (!player.getInventory().containsItem(995, price)) {
            player.sendMessage("You need " + Utils.format(price) + " coins to enchant the seed.");
            return;
        }
