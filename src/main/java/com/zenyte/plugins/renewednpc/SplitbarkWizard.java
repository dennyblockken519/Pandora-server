...
public class SplitbarkWizard extends NPCPlugin {
    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                options(new DialogueOption("What do you do here?", key(100)), new DialogueOption("What's that you're wearing?", key(200)), new DialogueOption("Can you make me some armour please?", key(300)), new DialogueOption("No thanks.", key(400)));
                ...
                npc("Split-bar armour is special armour for mages. It's much more resistant to physical attacks than normal robes.");
                ...
                npc("I'll charge you 1,000 coins for either gloves or boots,<br>6,000 coins for a hat,<br>32,000 coins for leggings,<br>and 37,000 for a top.").executeAction(key(140));
                player(200, "What's that you're wearing?");
                npc("This is split-bark armour, it's special armour for mages. It's much more resistant to physical attacks than normal robes.");
                npc("It's actually very easy for me to make, but I've been having trouble getting hold of the pieces.");
                options(new DialogueOption("Well good luck with that.", key(120)), new DialogueOption("Can you make me some?", key(130)));
    ...
    private enum SplitbarkArmour {
        HELM(ItemId.SPLITBARK_HELM, 2, 6000),
        BODY(ItemId.SPLITBARK_BODY, 4, 37000),
        LEGS(ItemId.SPLITBARK_LEGS, 3, 32000),
        GLOVES(ItemId.SPLITBARK_GAUNTLETS, 1, 1000),
        BOOTS(ItemId.SPLITBARK_BOOTS, 1, 1000);
        private final int product;
        private final int materialCount;
        private final int price;
        private static final List<SplitbarkArmour> values = Collections.unmodifiableList(Arrays.asList(values()));

        SplitbarkArmour(final int product, final int materialCount, final int price) {
    ...
    private final void sendMenu(@NotNull final Player player) {
        ...
        player.getDialogueManager().start(new SkillDialogue(player, "How many do you wish to buy?", list.toArray(new Item[0])) {
            ...
            public void run(int slotId, int amount) {
                final SplitbarkWizard.SplitbarkArmour constant = Objects.requireNonNull(Utils.findMatching(SplitbarkArmour.values, value -> value.product == list.get(slotId).getId()));
                if (!inventory.containsItem(ItemId.BARK, constant.materialCount) || !inventory.containsItem(ItemId.FINE_CLOTH, constant.materialCount) || !inventory.containsItem(ItemId.COINS_995, constant.price)) {
                    ...
                    player.getDialogueManager().start(new NPCChat(player, NpcId.WIZARD_3232, "You need " + quantifiedLabel + " bark, " + quantifiedLabel + " of fine cloth and " + Utils.format(constant.price) + " coins for a " + ItemDefinitions.getOrThrow(constant.product).getName().toLowerCase() + "."));
                    return;
                }
                ...
                player.getDialogueManager().start(new NPCChat(player, NpcId.WIZARD_3232, "There you go, enjoy your new armour!"));
            }
        });
    }
    ...
    @Override
    public int[] getNPCs() {
        return new int[]{NpcId.WIZARD_3232};
    }
}
