...
public class CaptainBentley extends NPCPlugin {
    ...
    private void sail(final Player player, final boolean piratesCove) {
        new FadeScreen(player, () -> player.setLocation(piratesCove ?
                PIRATES_COVE_LOCATION :
                LUNAR_ISLE_LOCATION)).fade(2);
    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                options("Sail to " + (piratesCove ? "Pirates' Cove?" : "Lunar Isle?"), "Yes, take me there.", "No, I'd rather not.")
                        .onOptionOne(() -> sail(player, piratesCove)).onOptionTwo(this::finish);
            }
        }));
