...
public class CombatBracelet extends ItemPlugin {
    ...
    private enum CombatBraceletItem {
        ONE(new Item(11124), new Item(11126), 1),
        TWO(new Item(11122), new Item(11124), 2),
        THREE(new Item(11120), new Item(11122), 3),
        FOUR(new Item(11118), new Item(11120), 4),
        FIVE(new Item(11974), new Item(11118), 5),
        SIX(new Item(11972), new Item(11974), 6);
        ...
        private static CombatBraceletItem get(@NotNull final Item item) {
            return Objects.requireNonNull(bracelets.get(item.getId()));
        }

        CombatBraceletItem(final Item item, final Item result, final int charges) {
            this.item = item;
            this.result = result;
    ...
    private static final class BraceletTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            ...
            final String message = charges == 1 ? "<col=7F00FF>You use your combat bracelet's last charge.</col>" : charges == 2 ? "<col=7F00FF>Your combat bracelet has one charge left.</col>" : "<col=7F00FF>Your combat bracelet has " + Utils.format(charges - 1) + " charges left.</col>";
        ...
        public BraceletTeleport(final Item item, final Container container, final int slotId, final Location destination) {
            this.item = item;
