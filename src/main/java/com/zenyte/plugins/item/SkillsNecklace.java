...
public class SkillsNecklace extends ItemPlugin {
    ...
    public void handle() {
        ...
        bind("Farming Guild", (player, item, container, slotId) -> new NecklaceTeleport(item, container, slotId, player.getSkills().getLevelForXp(Skills.FARMING) >= 45 ? new Location(1248, 3725, 0) : new Location(1248, 3719, 0)).teleport(player));
            ...
            public void handleClick(int clickedSlot) {
                ...
                switch (clickedSlot) {
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        new NecklaceTeleport(item, container, slotId, player.getSkills().getLevelForXp(Skills.FARMING) >= 45 ? new Location(1248, 3725, 0) : new Location(1248, 3719, 0)).teleport(player);
                        break;
                }
            }
    ...
    private enum SkillsNecklaceItem {
        ONE(new Item(11111), new Item(11113), 1),
        TWO(new Item(11109), new Item(11111), 2),
        THREE(new Item(11107), new Item(11109), 3),
        FOUR(new Item(11105), new Item(11107), 4),
        FIVE(new Item(11970), new Item(11105), 5),
        SIX(new Item(11968), new Item(11970), 6);
        ...
        private static SkillsNecklaceItem get(@NotNull final Item item) {
            return Objects.requireNonNull(amulets.get(item.getId()));
        }

        SkillsNecklaceItem(final Item item, final Item result, final int charges) {
            this.item = item;
            this.result = result;
    ...
    private static final class NecklaceTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            ...
            final String message = charges == 1 ? "<col=7F00FF>You use your skills necklace's last charge.</col>" : charges == 2 ? "<col=7F00FF>Your skills necklace has one charge left.</col>" : "<col=7F00FF>Your skills necklace has " + Utils.format(charges - 1) + " charges left.</col>";
        ...
        public NecklaceTeleport(final Item item, final Container container, final int slotId, final Location destination) {
            this.item = item;
