...
public class BurningAmulet extends ItemPlugin {
    ...
    private enum BurningAmuletItems {
        ONE(new Item(21175), null, 1),
        TWO(new Item(21173), new Item(21175), 2),
        THREE(new Item(21171), new Item(21173), 3),
        FOUR(new Item(21169), new Item(21171), 4),
        FIVE(new Item(21166), new Item(21169), 5);
        ...
        private static BurningAmuletItems get(@NotNull final Item item) {
            return Objects.requireNonNull(amulets.get(item.getId()));
        }

        BurningAmuletItems(final Item item, final Item result, final int charges) {
            this.item = item;
            this.result = result;
    ...
    private static final class BurningAmuletTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            ...
            final String message = charges == 1 ? "<col=7F00FF>You use your amulet's last charge.</col>" : charges == 2 ? "<col=7F00FF>Your amulet has one charge left.</col>" : "<col=7F00FF>Your amulet has " + Utils.format(charges - 1) + " charges left.</col>";
        ...
        public BurningAmuletTeleport(final Item item, final Container container, final int slotId, final Location destination) {
            this.item = item;
            this.container = container;
            this.slotId = slotId;
            this.destination = destination;
