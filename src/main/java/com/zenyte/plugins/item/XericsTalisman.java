...
public class XericsTalisman extends ItemPlugin implements PairedItemOnItemPlugin {
    @Override
    public void handle() {
        bind("Xeric's Look-out", (player, item, container, slotId) -> new TalismanTeleport(item, container, slotId, new Location(1579, 3531, 0)).teleport(player));
    ...
    private static final class TalismanTeleport implements Teleport {
        ...
        public TalismanTeleport(final Item item, final Container container, final int slotId, final Location destination) {
            this.item = item;
