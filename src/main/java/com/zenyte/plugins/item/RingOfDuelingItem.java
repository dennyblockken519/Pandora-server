...
public class RingOfDuelingItem extends ItemPlugin {
    ...
    public void handle() {
        bind("Rub", (player, item, container, slotId) -> {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    options("Where would you like to teleport to?", new DialogueOption("Duel Arena", () -> new RingTeleport(item, container, slotId, new Location(3315, 3235, 0)).teleport(player)), new DialogueOption("Castle Wars", () -> new RingTeleport(item, container, slotId, new Location(2440, 3090, 0)).teleport(player)), new DialogueOption("Clan Wars", () -> new RingTeleport(item, container, slotId, new Location(3388, 3160, 0)).teleport(player)), new DialogueOption("Nowhere"));
                }
            });
        });
        ...
        bind("Clan Wars", (player, item, container, slotId) -> new RingTeleport(item, container, slotId, new Location(3388, 3160, 0)).teleport(player));
    }
    ...
    private enum RingOfDueling {
        ONE(new Item(2566), null, 1),
        TWO(new Item(2564), new Item(2566), 2),
        THREE(new Item(2562), new Item(2564), 3),
        FOUR(new Item(2560), new Item(2562), 4),
        FIVE(new Item(2558), new Item(2560), 5),
        SIX(new Item(2556), new Item(2558), 6),
        SEVEN(new Item(2554), new Item(2556), 7),
        EIGHT(new Item(2552), new Item(2554), 8);
        ...
        @NotNull
        private static RingOfDueling get(@NotNull final Item item) {
        ...
        RingOfDueling(final Item item, final Item result, final int charges) {
            this.item = item;
            this.result = result;
    ...
    private static final class RingTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            ...
            final String message = charges == 1 ? "<col=7F00FF>Your ring of dueling crumbles to dust.</col>" : charges == 2 ? "<col=7F00FF>Your ring of dueling has one charge left..</col>" : "<col=7F00FF>Your ring of dueling has " + Utils.format(charges - 1) + " charges left.</col>";
        ...
        public RingTeleport(final Item item, final Container container, final int slotId, final Location destination) {
            this.item = item;
