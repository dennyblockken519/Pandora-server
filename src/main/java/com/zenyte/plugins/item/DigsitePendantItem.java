...
public class DigsitePendantItem extends ItemPlugin {
    ...
    private enum Pendant {
        ONE(new Item(11190), null, 1),
        TWO(new Item(11191), new Item(11190), 2),
        THREE(new Item(11192), new Item(11191), 3),
        FOUR(new Item(11193), new Item(11192), 4),
        FIVE(new Item(11194), new Item(11193), 5);
        ...
        private static Pendant get(@NotNull final Item item) {
            return pendants.get(item.getId());
        }

        Pendant(final Item item, final Item result, final int charges) {
            this.item = item;
            this.result = result;
    ...
    private static final class PendantTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            ...
            final String message = charges == 1 ? "<col=7F00FF>You use your pendant's last charge.</col>" : charges == 2 ? "<col=7F00FF>Your pendant has one charge left.</col>" : "<col=7F00FF>Your pendant has " + Utils.format(charges - 1) + " charges left.</col>";
        ...
        public PendantTeleport(final Item item, final Container container, final int slotId, final Location destination) {
            this.item = item;
