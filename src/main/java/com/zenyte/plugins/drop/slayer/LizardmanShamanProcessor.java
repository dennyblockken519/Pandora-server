...
public class LizardmanShamanProcessor extends DropProcessor {
    @Override
    public void attach() {
        appendDrop(new DisplayedDrop(13576, 1, 1, 2500));
        appendDrop(new DisplayedDrop(13392, 1, 1) {
            public double getRate(final Player player, final int id) {
                return DiaryReward.RADAS_BLESSING1.eligibleFor(player) ? 125 : 250;
            }
        });
    ...
    public Item drop(final NPC npc, final Player killer, final Drop drop, final Item item) {
        if (!drop.isAlways()) {
            ...
            } else if (random(DiaryReward.RADAS_BLESSING1.eligibleFor(killer) ? 125 : 250) == 0) {
