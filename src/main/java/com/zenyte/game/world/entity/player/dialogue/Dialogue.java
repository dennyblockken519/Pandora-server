...
public abstract class Dialogue {
    ...
    public ItemMessage item(final int stage, final Item item, final String message) {
        final ItemMessage msg = new ItemMessage(item, message);
        key = stage + 1;
        dialogue.put(stage, msg);
        return msg;
    }
    ...
    public OptionMessage options(final String title, final DialogueOption... options) {
        DialogueOption[] cleanedOptions = ArrayUtils.removeAllOccurences(options, null);
    ...
    public void setOnBuild(final Runnable onBuild) {
