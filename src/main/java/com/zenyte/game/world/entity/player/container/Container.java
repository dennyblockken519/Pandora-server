...
public class Container {
    ...
    protected Int2ObjectLinkedOpenHashMap<Item> items;
     ...
     *
     * @param container the container whose variables to copy.
     */
     ...
     * @param item the object to set it to, or null if removing.
     */
    public void set(final int slot, final Item item) {
        ...
        if (item == null) {
            ...
            final float itemWeight = item.getDefinitions().getWeight();
            if (itemWeight > 0) {
                weight += itemWeight;
            }
            modifiedSlots.add(slot);
     ...
     *
     * @param id the id of the item to calculate the amount of.
     ...
     *
     * @param id the id of the item that's requested.
     ...
     *
     * @param id the item id that's being requested.
    ...
    public int[] getSlotsOf(final int id) {
        ...
        final int[] array = list.toArray(new int[list.size()]);
        Arrays.sort(array);
        return array;
    }
     ...
     *
     * @param slot the slot which to check.
     ...
     * returns false.
     *
     * @param id     the id of the item that's being checked for.
     * @param amount the minimum amount of the given item for the method to return as true.
    ...
    public void shift() {
        ...
        final Int2ObjectOpenHashMap<Item> shiftedMap = new Int2ObjectOpenHashMap<>(itemsSize);
        int index = 0;
    ...
    public MultiResult add(final Item... items) {
        final ArrayList<ContainerResult> results = new ArrayList<>(items.length);
        for (int i = 0; i < items.length; i++) {
            final Item item = items[i];
            results.add(this.add(item));
        }
        final MultiResult multiresult = new MultiResult(ContainerState.ADD, results.toArray(new ContainerResult[results.size()]));
        return multiresult;
    }
    ...
    public MultiResult remove(final Item... items) {
        ...
        final MultiResult multiresult = new MultiResult(ContainerState.REMOVE, results.toArray(new ContainerResult[results.size()]));
        return multiresult;
    }
     ...
     *
     * @param slot   the slot in the container that needs to be removed.
     * @param amount the amount to remove from that given slot. If the amount is less than there is of that item in the slot, only the
     ...
     *
     * @param player the player for whom the container is being updated.
     */
    ...
    private void recalculateWeight() {
        ...
        for (final Int2ObjectMap.Entry<Item> entry : this.items.int2ObjectEntrySet()) {
            ...
            final float weight = item.getDefinitions().getWeight();
     ...
     *
     * @param slots the slots that need to be force updated.
     */
     ...
     *
     * @param player    the player who is depositing the item to this container - variable can be null; if it isn't null an something goes wrong,
     ...
     *
     * @param player    the player who is withdrawing the item to this container - variable can be null; if it isn't null an something goes wrong,
     *                  e.g. there's not enough space, the player is notified about it.
    ...
    public void withdraw(final Player player, final Container container, int slot, int amount) {
        ...
        if (!item.hasAttributes() && (definitions.isStackable() || policy == ContainerPolicy.ALWAYS_STACK)) {
            ...
            if ((amount > realAmount ? realAmount : amount) + existingAmount < 0) {
    ...
    public void setPolicy(final ContainerPolicy policy) {
        this.policy = policy;
    }
