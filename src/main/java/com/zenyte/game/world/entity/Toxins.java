...
public final class Toxins {
    ...
    public void applyToxin(final ToxinType type, final int damage) {
        applyToxin(type, damage, false);
    }

    public void applyToxin(final ToxinType requestedType, final int damage, final boolean applyDamage) {
        ...
        if (entity.getEntityType() == EntityType.PLAYER) {
            ...
            if (CombatUtilities.isWearingSerpentineHelmet(player)) return;
        ...
        if (type == ToxinType.VENOM) {
            ...
            if (!isVenomed()) {
                ...
                this.type = type;
                if (applyDamage) {
                    entity.applyHit(new Hit(damage, HitType.VENOM));
                }
                refresh();
            }
            ...
            if (!isPoisoned()) {
                ...
                this.type = type;
                if (applyDamage) {
                    entity.applyHit(new Hit(damage, HitType.POISON));
                }
                refresh();
            }
    ...
    private void applyDisease() {
        ...
        if (entity.getEntityType() == EntityType.PLAYER) {
            ...
            if (randomSkill == Skills.PRAYER) {
                player.getPrayerManager().drainPrayerPoints(hit);
            } else if (randomSkill == Skills.HITPOINTS) {
    ...
    public void setType(final ToxinType type) {
        this.type = type;
    }
    ...
    public void setDiseaseCycle(final int diseaseCycle) {
        this.diseaseCycle = diseaseCycle;
    }

    public void setDiseaseInterval(final int diseaseInterval) {
