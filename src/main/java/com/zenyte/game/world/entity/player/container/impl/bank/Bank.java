...
public final class Bank {
    ...
    public ContainerResult add(final Item requestedItem, int tab) {
        ...
        final Predicate<Player> predicate = predicateMap.get(item.getId());
        if (predicate != null) {
    ...
    public ContainerResult remove(final int slot, final Item item, final boolean placeholder) {
        final ContainerResult result = container.remove(item, placeholder || item.getId() != 20594 && getSetting(BankSetting.ALWAYS_PLACEHOLDER) == 1, player);
        if (slot == -1) {
            return result;
        }
    ...
    public void switchItem(final BankSwitchType type, final int fromSlot, int toSlot) {
        if (type.equals(BankSwitchType.ITEM_TO_ITEM)) {
            ...
            final ArrayList<Item> list = new ArrayList<>(container.getSize());
            int i;
            int fromStartIndex;
            int toStartIndex;
    ...
    public void collapseTab(final int tab) {
        ...
        if (tab == MAIN_TAB) {
            final ArrayList<Item> mainTab = new ArrayList<>(tabSizes[0]);
        ...
        final ArrayList<Item> list = new ArrayList<>(tabSizes[tab]);
    ...
    public void revalidate() {
        ...
        if (slots.firstInt() != length) {
            final Int2ObjectAVLTreeMap<Item> map = new Int2ObjectAVLTreeMap<>(container.getItems());
    ...
    public enum QuantitySelector {
        ...
        QuantitySelector(final int value) {
            this.value = value;
        }
    }
}
