...
public final class BankContainer extends Container {
    ...
    public BankContainer(final Bank bank, final ContainerPolicy policy, final ContainerType type) {
        super(policy, type, Optional.of(bank.getPlayer()));
        this.bank = bank;
    }
    ...
    public ContainerResult add(final Item requestedItem) {
        ...
        final Predicate<Player> predicate = Bank.predicateMap.get(item.getId());
        if (predicate != null) {
            if (!predicate.test(null)) {
                final ContainerResult res = new ContainerResult(item, ContainerState.ADD);
                res.setResult(RequestResult.FAILURE);
                return res;
            }
        }
     ...
     *
     * @param player    the player who is depositing the item to this container - variable can be null; if it isn't null an something goes
     ...
     * however it may be limited as containers have different policies and sizes.
     *
     * @param player    the player who is withdrawing the item to this container - variable can be null; if it isn't null an something goes
     *                  wrong, e.g. there's not enough space, the player is notified about it.
	...
	public int getContainerSize() {
		return bank.getPlayer().getPerkManager().isValid(PerkWrapper.FILL_THE_BANK) ? 834 : 634;
	}
*/
}
