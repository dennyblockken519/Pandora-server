...
public class Region {
    ...
    public void addMultiZone(final int chunkXInRegion, final int chunkYInRegion, final int chunkZ, final int copiedChunkHash) {
        final int copiedChunkX = copiedChunkHash & 2047;
        final int copiedChunkY = copiedChunkHash >> 11 & 2047;
        final int copiedChunkZ = copiedChunkHash >> 22 & 3;
        final int x = copiedChunkX << 3;
        final int y = copiedChunkY << 3;
        final Polygon chunkPolygon = new RSPolygon(new int[][]{new int[]{x, y}, new int[]{x + 8, y}, new int[]{x + 8, y + 8}, new int[]{x, y + 8}}, copiedChunkZ).getPolygon();
        final Area chunkArea = new Area(chunkPolygon);
        final int regionChunkX = (regionId >> 8) << 3;
        final int regionChunkY = (regionId & 255) << 3;
        final List<List<Area>> allPolygons = MultiwayArea.getPolygons();
        final List<Area> polygons = allPolygons.get(copiedChunkZ);
        for (int i = polygons.size() - 1; i >= 0; i--) {
            ...
            if (!area.isEmpty()) {
                final int xOffset = ((regionChunkX + chunkXInRegion) << 3) - x;
                final int yOffset = ((regionChunkY + chunkYInRegion) << 3) - y;
                final Chunk chunk = World.getChunk(Chunk.getChunkHash(regionChunkX + chunkXInRegion, regionChunkY + chunkYInRegion, chunkZ));
                final Area repositionedArea = getRepositionedArea(area, xOffset, yOffset);
                final List<EfficientArea> efficientAreas = MultiwayArea.addDynamicMultiArea(repositionedArea, chunkZ);
                for (final EfficientArea efficientArea : efficientAreas) {
            ...
            if (mapBuffer != null) {
                ...
                for (int plane = 0; plane < 4; plane++) {
                    for (int x = 0; x < 64; x++) {
                        for (int y = 0; y < 64; y++) {
                            while (true) {
                                final int value = mapBuffer.readUnsignedByte();
                                if (value == 0) {
                                    break;
                                } else if (value == 1) {
                                    mapBuffer.readByte();
            ...
            if (landBuffer != null) {
                ...
                while ((incr = landBuffer.readHugeSmart()) != 0) {
                    ...
                    while ((incr2 = landBuffer.readUnsignedSmart()) != 0) {
                        ...
                        spawnObject(new WorldObject(objectId, type, rotation, localX + baseX, localY + baseY, objectPlane), objectPlane, localX, localY, true, true);
                    }
                }
            }
    ...
    public boolean containsSpawnedObject(final WorldObject object) {
        ...
        final short hash = (short) (object.getXInRegion() | object.getYInRegion() << 6 | OBJECT_SLOTS[object.getType()] << 12 | object.getPlane() << 14);
    ...
    public WorldObject getObjectWithType(final int locationZ, final int locationX, final int locationY, final int type) {
        if (objects == null) {
            return null;
        }
        final int x = locationX & 63;
