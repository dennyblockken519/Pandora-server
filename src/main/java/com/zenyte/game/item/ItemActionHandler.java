...
public class ItemActionHandler {
    ...
    public static final Int2ObjectOpenHashMap<ItemPlugin> INT_ACTIONS = new Int2ObjectOpenHashMap<>();
    ...
    public static void handle(final Player player, final int itemId, final int slotId, final int option) {
        ...
        final String optionName = Objects.requireNonNull(item.getDefinitions().getOption(option - 1));
    ...
    public static void add(final Class<?> c) {
        try {
            ...
            for (final int item : action.getItems()) {
                final ItemPlugin old = INT_ACTIONS.put(item, action);
                if (old != null) {
