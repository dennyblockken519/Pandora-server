...
public class OpPlayerDecoder implements ClientProtDecoder<OpPlayerEvent> {
    public static final int[] OPCODES = {ClientProt.OPPLAYER1.getOpcode(), ClientProt.OPPLAYER2.getOpcode(), ClientProt.OPPLAYER3.getOpcode(), ClientProt.OPPLAYER4.getOpcode(), ClientProt.OPPLAYER5.getOpcode(), ClientProt.OPPLAYER6.getOpcode(), ClientProt.OPPLAYER7.getOpcode(), ClientProt.OPPLAYER8.getOpcode()};

    @Override
    public OpPlayerEvent decode(Player player, int opcode, RSBuffer buffer) {
        int index = -1;
        boolean run = false;
