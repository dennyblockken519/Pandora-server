...
public class PacketDispatcher {
    ...
    public void sendLogout() {
        player.setLoggedOut(true);
        player.setLogoutCount(100);
    ...
    public void initFriendsList() {
        ...
        final ArrayList<UpdateFriendList.FriendEntry> list = new ArrayList<>(friends.size());
    ...
    public void initIgnoreList() {
        ...
        final ArrayList<UpdateIgnoreList.IgnoreEntry> list = new ArrayList<>(ignores.size());
    ...
    public void sendStaticMapRegion() {
        player.send(new RebuildNormal(player));
    }

    public void sendDynamicMapRegion() {
        player.send(new RebuildRegion(player));
    }
    ...
    public void sendGraphics(final Graphics graphics, final Location location) {
        player.sendZoneUpdate(location.getX(), location.getY(), new SpotAnimSpecific(location, graphics));
    }
    ...
    public void sendAreaSoundEffect(final Location tile, final SoundEffect sound) {
        //If muted, skip the area sound.
        if (player.getIntSetting(Setting.AREA_SOUND_VOLUME) == 4) {
            return;
        }
    ...
    public void playJingle(final int song) {
        player.send(new MidiSongLocation(song));
    }
