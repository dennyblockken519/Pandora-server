...
public class ShopInventoryInterface extends Interface {
    ...
    public void open(Player player) {
        ...
        dispatcher.sendComponentSettings(getInterface(), getComponent("Interact with item"), 0, ContainerType.INVENTORY.getSize(), CLICK_OP1, CLICK_OP2, CLICK_OP3, CLICK_OP4, CLICK_OP5, CLICK_OP10);
    }
    ...
    protected void build() {
        bind("Interact with item", ((player, slotId, itemId, option) -> {
            final Object shopAttr = player.getTemporaryAttributes().get("Shop");
            if (!(shopAttr instanceof Shop shop)) {
                return;
            }
            ...
            if (op.is(ItemOption.VALUE)) {
                ...
                if ((item.getId() == 995 || !item.isTradable()) && !shop.getName().toLowerCase().contains("grace's graceful clothing")) {
    ...
    enum ItemOption {
        ...
        private static ItemOption of(final int option) {
            final ShopInventoryInterface.ItemOption constant = Utils.findMatching(values, value -> value.optionId == option);
            if (constant == null) {
