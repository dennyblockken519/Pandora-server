    ...
    private static void create(final Player player, final WorldObject object) {
        ...
        player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                options("Are you sure you want to make the Ferocious Gloves?<br>" + Colour.RED.wrap("(This will make them untradeable)"), "Yes", "No")
                        .onOptionOne(() -> player.getActionManager().setAction(new ModifyFerociousGloves(object, true)));
            }
        });
    }

    private static void unassemble(@NotNull final Player player, final WorldObject object) {
        ...
        player.getDialogueManager().start(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                options("Are you sure you want to revert the gloves into leather?<br>" + Colour.RED.wrap("(This will make them tradeable)"), "Yes", "No")
                        .onOptionOne(() -> player.getActionManager().setAction(new ModifyFerociousGloves(object, false)));
            }
        });
    }
