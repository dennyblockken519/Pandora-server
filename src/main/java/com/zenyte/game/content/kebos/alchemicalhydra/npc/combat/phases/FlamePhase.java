...
public class FlamePhase implements HydraPhaseSequence {
    ...
    private Location getNextFollowLocation(final Location flame, final Location player) {
        ...
        final int direction = Utils.getFaceDirection(player.getX() - flame.getX(), player.getY() - flame.getY());
        final int entityDirection = Entity.getRoundedDirection(direction, 0);
        final Direction directionConstant = Utils.findMatching(Direction.values, dir -> dir.getNPCDirection() == entityDirection);
    ...
    private final List<WorldObject> spawnBlockingObjects(@NotNull final Rectangle outer, final int plane) {
        ...
        final ObjectArrayList<WorldObject> list = new ObjectArrayList<>();
    ...
    public void handleIngoingHit(final AlchemicalHydra hydra, final Hit hit) {
        final int hitpoints = hydra.getHitpoints();
    ...
    public boolean isPerformingSpecialAttack() {
        return this.performingSpecialAttack;
    }

    public boolean isWalkingToMiddle() {
        return this.walkingToMiddle;
    }
    ...
    private class FireGraphics {
        ...
        public FireGraphics(final int ticks, final Graphics graphics, final Location tile) {
            this.ticks = ticks;
