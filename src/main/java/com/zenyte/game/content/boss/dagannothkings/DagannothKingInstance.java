 ...
 */
public class DagannothKingInstance extends DynamicArea implements CannonRestrictionPlugin, DeathPlugin, LogoutPlugin, DropPlugin, LootBroadcastPlugin {
    private static final Logger log = LoggerFactory.getLogger(DagannothKingInstance.class);
    private static final NPCSpawn[] spawns = new NPCSpawn[]{new NPCSpawn(5948, 2903, 4435, 0, Direction.SOUTH, 2), new NPCSpawn(5948, 2903, 4435, 0, Direction.SOUTH, 4), new NPCSpawn(5962, 2898, 4439, 0, Direction.SOUTH, 3), new NPCSpawn(5962, 2898, 4446, 0, Direction.SOUTH, 4), new NPCSpawn(5948, 2897, 4446, 0, Direction.SOUTH, 2), new NPCSpawn(5948, 2897, 4455, 0, Direction.SOUTH, 2), new NPCSpawn(5961, 2898, 4455, 0, Direction.SOUTH, 2), new NPCSpawn(5963, 2901, 4458, 0, Direction.SOUTH, 4), new NPCSpawn(5947, 2901, 4459, 0, Direction.SOUTH, 2), new NPCSpawn(5946, 2903, 4464, 0, Direction.SOUTH, 4), new NPCSpawn(5961, 2910, 4434, 0, Direction.SOUTH, 2), new NPCSpawn(2265, 2905, 4443, 0, Direction.SOUTH, 3), new NPCSpawn(5947, 2910, 4463, 0, Direction.SOUTH, 5), new NPCSpawn(5962, 2909, 4464, 0, Direction.SOUTH, 5), new NPCSpawn(5948, 2917, 4433, 0, Direction.SOUTH, 3), new NPCSpawn(5962, 2919, 4436, 0, Direction.SOUTH, 8), new NPCSpawn(2266, 2913, 4456, 0, Direction.SOUTH, 3), new NPCSpawn(2267, 2920, 4445, 0, Direction.SOUTH, 3), new NPCSpawn(5962, 2922, 4461, 0, Direction.SOUTH, 4), new NPCSpawn(5946, 2927, 4457, 0, Direction.SOUTH, 2), new NPCSpawn(5947, 2918, 4463, 0, Direction.SOUTH, 2), new NPCSpawn(5946, 2923, 4464, 0, Direction.SOUTH, 2), new NPCSpawn(5948, 2920, 4464, 0, Direction.SOUTH, 2), new NPCSpawn(5962, 2928, 4439, 0, Direction.SOUTH, 2), new NPCSpawn(5962, 2930, 4441, 0, Direction.SOUTH, 2), new NPCSpawn(5961, 2930, 4445, 0, Direction.SOUTH, 5), new NPCSpawn(5946, 2928, 4453, 0, Direction.SOUTH, 2), new NPCSpawn(5963, 2930, 4450, 0, Direction.SOUTH, 3)};
    private final String username;

    protected DagannothKingInstance(final String username, AllocatedArea allocatedArea, int copiedChunkX, int copiedChunkY) {
    ...
    public void constructed() {
        final int[][] poly = Spinolyp.polyBounds;
        final int[][] array = new int[poly.length][];
        for (int i = 0; i < poly.length; i++) {
            array[i] = new int[]{getX(poly[i][0]), getY(poly[i][1])};
        }
        final RSPolygon polygon = new RSPolygon(array);
        for (final NPCSpawn spawn : spawns) {
            final int id = spawn.getId();
            if (id == 5947 || id == 5961 || id == 5963) {
                new InstanceSpinolyp(polygon, id, getLocation(spawn.getX(), spawn.getY(), spawn.getZ()), spawn.getDirection(), spawn.getRadius()).spawn();
                continue;
            }
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (ticks == 0) {
                    ...
                    player.getDeathMechanics().service(ItemRetrievalService.RetrievalServiceType.HAGAVIK, source);
