...
public class Dusk extends NPC implements CombatScript {
    ...
    public int attack(Entity target) {
        if (!(target instanceof Player player) || dying || isDead()) {
    ...
    public void prisonAttack(final boolean first) {
        ...
        while (true) {
            ...
            optionalCenter = Utils.findEmptySquare(randomPoint, 30, 5, Optional.of(l -> !Utils.collides(l, chamberSize, getLocation(), 5, finalTilesDistance) && boundary.contains(l.getX(), l.getY())));
    ...
    public void setDawn(final Dawn dawn) {
        this.dawn = dawn;
    }

    public void setExplosiveAttack(final boolean explosiveAttack) {
        this.explosiveAttack = explosiveAttack;
    }

    public long getPrisonAttackDelay() {
        return this.prisonAttackDelay;
    }

    public boolean isPerformingPrisonAttack() {
        return this.performingPrisonAttack;
    }
}
