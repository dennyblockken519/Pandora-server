...
public class SextantInterface extends Interface {
    ...
    private final void refreshSextantPosition(@NotNull final Player player) {
        ...
        player.send(new IfSetPosition(id, getComponent("Grass"), BASE_VIEW_HORIZONTAL_COORD, getGrassVerticalPosition(player)));
        player.send(new IfSetPosition(id, getComponent("Sun"), BASE_VIEW_HORIZONTAL_COORD, getSunVerticalPosition(player)));
        player.send(new IfSetAnim(id, getComponent("Arm"), SextantPosition.BASE_ARM_POSITION + armPosition));
        player.send(new IfSetAnim(id, getComponent("Frame"), SextantPosition.BASE_FRAME_POSITION + sextantPosition.getFramePosition()));
        player.send(new IfSetAnim(id, getComponent("Index mirror"), SextantPosition.BASE_FRAME_POSITION + sextantPosition.getFramePosition()));
    }
    ...
    protected void build() {
        ...
        bind("Get location", player -> {
            ...
            if (!inventory.containsItem(ItemId.WATCH, 1) || !inventory.containsItem(ItemId.CHART, 1)) {
                player.getDialogueManager().start(new PlainChat(player, "You need a watch and navigator's chart to work out your location."));
                return;
            }
