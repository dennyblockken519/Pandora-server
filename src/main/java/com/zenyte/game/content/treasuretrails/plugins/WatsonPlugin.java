...
public class WatsonPlugin extends NPCPlugin {
    ...
    private final boolean deposit(@NotNull final Player player, @NotNull final ClueItem type, final boolean plural) {
        assert ClueItem.EASY.ordinal() == 1;//Ensure that the starting index is valid.
        final Inventory inventory = player.getInventory();
        ...
        if (success) {
            ...
            if (completed) {
                ...
                player.getDialogueManager().start(new Dialogue(player, 7304) {
                    @Override
                    public void buildDialogue() {
                        item(new Item(ClueItem.MASTER.getScrollBox()), "You hand over the clue scroll" + (plural ? 's' : Strings.EMPTY) + " and receive a master scroll box in return.");
                    }
                });
                return true;
            }
