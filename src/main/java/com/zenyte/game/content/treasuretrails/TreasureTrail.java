...
public class TreasureTrail {
    ...
    public static boolean isMimicEnabled(@NotNull final Player player) {
        return player.getNumericAttribute(Constants.MIMIC_STATUS).intValue() == 0;
    }

    public static void setMimicStatus(@NotNull final Player player, final boolean enabled) {
        player.addAttribute(Constants.MIMIC_STATUS, enabled ? 0 : 1);
    }

    private static Optional<ClueLevel> getLevel(@NotNull final Item item) {
        final ClueItem clueItem = ClueItem.getMap().get(item.getId());
        return Optional.ofNullable(clueItem == null ? null : clueItem.getLevel());
    }

    public static void setClue(@NotNull final Item item, final Clue clue) {
    ...
    public static Clue setRandomClue(@NotNull final Item item) {
        final ClueLevel level = Objects.requireNonNull(getLevel(item).orElseThrow(RuntimeException::new));
    ...
    public static List<CrypticClue> getCrypticClues(@NotNull final Item item) {
        final ClueLevel level = Objects.requireNonNull(getLevel(item).orElseThrow(RuntimeException::new));
        assert level == ClueLevel.MASTER;
        final Object attr = item.getAttribute(Constants.CLUE_SCROLL_NAME);
        assert attr instanceof List;
        @SuppressWarnings("unchecked") final List<String> list = (List<String>) attr;
        final ObjectArrayList<CrypticClue> typedList = new ObjectArrayList<>();
    ...
    public static int getCrypticCluesStage(@NotNull final Item item) {
        final ClueLevel level = Objects.requireNonNull(getLevel(item).orElseThrow(RuntimeException::new));
        assert level == ClueLevel.MASTER;
        return item.getNumericAttribute(Constants.CLUE_SCROLL_STAGE).intValue();
    }

    private static boolean advance(@NotNull final Item item) {
    ...
    private static <T> boolean findAndProgress(@NotNull final Player player, @NotNull final Class<T> clazz, @NotNull final Predicate<T> predicate, Consumer<ClueEntry> progressConsumer) {
        final Optional<ClueEntry> entry = findClueScroll(player, clazz, predicate);
        entry.ifPresent(progressConsumer);
        return entry.isPresent();
    }

    public static void progressSherlockTask(@NotNull final Player player, @Nullable final Predicate<Player> predicate, @NotNull final SherlockTask task) {
    ...
    public static Optional<ClueEntry> findSherlockClue(@NotNull final Player player) {
        return findClueScroll(player, SherlockRequest.class, __ -> true);
    }

    public static void progressCharlieTask(@NotNull final Player player, @NotNull final CharlieTask task) {
    ...
    public static Optional<ClueEntry> findFaloClue(@NotNull final Player player) {
        return findClueScroll(player, FaloTheBardChallenge.class, predicate -> true);
    }

    private static <T> Optional<ClueEntry> findClueScroll(@NotNull final Player player, @NotNull final Class<T> clazz, @NotNull final Predicate<T> predicate) {
    ...
    public static void progress(@NotNull final Player player, @NotNull final ClueEntry entry, @NotNull final Optional<Dialogue> optionalDialogue) {
    ...
    public static void progress(@NotNull final Player player, @NotNull final Item item, final int slot, @NotNull final Optional<Dialogue> optionalDialogue) {
        progress(player, item, slot, optionalDialogue, -1);
    }

    public static void progressTripleCryptic(@NotNull final Player player, @NotNull final Item item) {
    ...
    private static void progress(@NotNull final Player player, @NotNull final Item item, final int slot, @NotNull final Optional<Dialogue> optionalDialogue, final int index) {
        if (index != -1) {
    ...
    private static boolean onFaladorParkBridge(@NotNull final Location location) {
    ...
    public static void playSong(@NotNull final Player player, @NotNull final String songName) {
    ...
    public static boolean containsMusicClue(@NotNull final Player player) {
        return findClueScroll(player, SongRequest.class, __ -> true).isPresent();
    }

    public static boolean dig(@NotNull final Player player) {
        ...
        final MutableObject<TreasureGuardianNPC> guardian = new MutableObject<>();
    ...
    private static boolean interruptedByGuardians(@NotNull final Player player, @Nullable final TreasureGuardianNPC type) {
        if (type == null) {
            return false;
        }
        ...
        if (guardianAttr == null) {
            ...
            final ArrayList<TreasureGuardian> list = new ArrayList<>();
    ...
    public static Optional<HotColdResult> getHotColdResult(@NotNull final Player player, final boolean beginner) {
        ...
        for (final String constantName : list) {
            ...
            final Optional<DeviceTemperature> temperature = DeviceTemperature.get(clueScroll.level() == ClueLevel.BEGINNER, distance);
            return Optional.of(new HotColdResult(slot.getAsInt(), clueItem, hc, distance, temperature.orElse(null)));
        }
        throw new IllegalStateException();
    }

    private static OptionalInt findHotColdClue(@NotNull final Player player, final boolean beginner) {
    ...
    @NotNull
    private static Location getSpawnTile(@NotNull final Location tile) {
        return getSpawnTile(tile, null);
    }

    @NotNull
    private static Location getSpawnTile(@NotNull final Location tile, @Nullable final Predicate<Position> predicate) {
     ...
     */
    private static boolean isClueScroll(@Nullable final Item item) {
        if (item == null) {
            return false;
        }
        final ClueItem clueItem = ClueItem.getMap().get(item.getId());
    ...
    @NotNull
    private static Item[] buildEquipmentArray(@NotNull final Player player) {
        final Item[] equipment = new Item[ContainerType.EQUIPMENT.getSize()];
    ...
    public static void emoteChallenge(@NotNull final Player player, @NotNull final Emote emote) {
    ...
    public static boolean speakWithUri(@NotNull final Player player, @NotNull final UriNPC npc) {
        final MutableObject<Item[]> mutableEquipment = new MutableObject<>();
    ...
    public static boolean searchKeyObject(@NotNull final Player player, @NotNull final WorldObject object, @NotNull final String option) {
    ...
    public static boolean search(@NotNull final Player player, @NotNull final WorldObject object, @NotNull final String option) {
    ...
    public static boolean searchWithkey(@NotNull final Player player, @NotNull final WorldObject object, @NotNull final String option) {
        return searchKeyObject(player, object, option);
    }

    private static boolean talkChallenge(@NotNull final Player player, @NotNull final NPC npc) {
    ...
    private static boolean talkPuzzleBox(@NotNull final Player player, @NotNull final NPC npc) {
        final int transmogrifiedId = player.getTransmogrifiedId(npc.getDefinitions(), npc.getId());
    ...
    private static boolean talkLightBox(@NotNull final Player player, @NotNull final NPC npc) {
        final int transmogrifiedId = player.getTransmogrifiedId(npc.getDefinitions(), npc.getId());
    ...
    private static OptionalInt findPuzzleBox(@NotNull final Player player, @NotNull final String clueName) {
        final Inventory inventory = player.getInventory();
        for (final Int2ObjectMap.Entry<Item> entry : inventory.getContainer().getItems().int2ObjectEntrySet()) {
            final Item item = entry.getValue();
    ...
    private static OptionalInt findLightBox(@NotNull final Player player, @NotNull final String clueName) {
        final Inventory inventory = player.getInventory();
        for (final Int2ObjectMap.Entry<Item> entry : inventory.getContainer().getItems().int2ObjectEntrySet()) {
            final Item item = entry.getValue();
    ...
    public static boolean withoutHotColdClue(@NotNull final Player player, final boolean beginner) {
        final Inventory inventory = player.getInventory();
        for (final Int2ObjectMap.Entry<Item> entry : inventory.getContainer().getItems().int2ObjectEntrySet()) {
            final Item item = entry.getValue();
            if (item == null) {
    ...
    public static List<String> getCluesList(@NotNull final Item item) {
        final Object attr = item.getAttribute(Constants.CLUE_SCROLL_NAME);
        if (!(attr instanceof List)) return null;
        return (List<String>) attr;
    }

    public static OptionalInt getKey(@NotNull final Player player, @NotNull final NPC npc) {
    ...
    public static boolean talk(@NotNull final Player player, @NotNull final NPC npc) {
    ...
    public static void kill(@NotNull final Player player, @NotNull final NPC npc) {
    ...
    public static void continueSherlockTask(@NotNull final Player player, @NotNull final Item item, final int slot) {
        final List<String> list = getCluesList(item);
        if (list == null) {
            return;
        }
    ...
    public static void continueCharlieTask(@NotNull final Player player, @NotNull final Item item, final int slot) {
    ...
    public static Object2ObjectMap<String, Clue> getClues() {
