...
public class ClueCasket extends ItemPlugin {
    static void open(@NotNull final Player player, @NotNull final Item item, @NotNull final ClueLevel tier) {
        final ClueRewardTable rewards = Objects.requireNonNull(ClueReward.getTable(item.getId()));
        final List<Item> loot = rewards.roll(player.inArea(Entrana.class));
