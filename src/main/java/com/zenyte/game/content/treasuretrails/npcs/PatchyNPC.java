...
public class PatchyNPC extends NPCPlugin implements ItemOnNPCAction {
    ...
    public void handleItemOnNPCAction(Player player, Item item, int slot, NPC npc) {
        final PatchyNPC.SewableClothing clothing = Objects.requireNonNull(Utils.findMatching(SewableClothing.values, v -> v.sewnItem == item.getId()));
    ...
    private enum SewableClothing {
        ...
        SewableClothing(final int baseItem, final int attachmentItem, final int sewnItem) {
