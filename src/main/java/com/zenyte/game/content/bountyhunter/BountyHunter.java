...
public class BountyHunter {
     ...
     */
    private final void addSkipEntry(@NotNull final SkipEntry entry) {
        if (lastSkips == null) {
            lastSkips = new ObjectOpenHashSet<>();
        }
        lastSkips.add(entry);
     ...
     *
     * @return list of the skips that have occurred within the last 30 minutes.
     */
    private final int getLastSkipsCount() {
        filterSkips();
     ...
     */
    private final void filterSkips() {
        if (lastSkips == null) {
            return;
        }
