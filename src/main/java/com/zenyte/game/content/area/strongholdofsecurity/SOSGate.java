...
public final class SOSGate implements ObjectAction {
    ...
    private static void pass(final Player player, final WorldObject object) {
     ...
     * West/East
     */
    private static void interact(final Player player, final Location tile, final WorldObject object, final boolean question) {
        if (question) {
            ...
            final SOSGate.Gate gate = Utils.findMatching(Gate.VALUES, g -> ArrayUtils.contains(g.gateIds, object.getId()));
            if (gate == null) {
    ...
    private static void pass(final Player player, final Location tile) {
        player.lock();
    ...
    private enum Gate {
        GATE_OF_WAR(2494, new int[]{19206, 19207}),
        RICKETY_DOOR(2495, new int[]{17009, 17100}),
        OOZING_BARRIER(2496, new int[]{23653, 23654}),
        PORTAL_OF_DEATH(2497, new int[]{23727, 23728});
        private static final Gate[] VALUES = values();
        private final int npcId;
        private final int[] gateIds;

        Gate(final int npcId, final int[] gateIds) {
            this.npcId = npcId;
