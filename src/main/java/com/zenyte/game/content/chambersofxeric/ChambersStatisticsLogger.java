...
public class ChambersStatisticsLogger {
    ...
    @Subscribe
    public static void onShutdown(final ServerShutdownEvent event) {
        instance.shutdown();
    }

    static void record(@NotNull final Raid raid) {
        if (raid.isRecorded()) {
            return;
        }
    ...
    private String compose(@NotNull final Raid raid) {
        ...
        if (originalPlayers.size() > 0) {
            ...
            if (originalPlayers.size() > players.size()) {
                ...
                for (final String originalPlayer : originalPlayers) {
                    final Player existingPlayer = Utils.findMatching(players, player -> player.getUsername().equals(originalPlayer));
        ...
        if (map != null) {
            final ObjectArrayList<RaidArea> chunks = new ObjectArrayList<>(map.getRaidChunks());
