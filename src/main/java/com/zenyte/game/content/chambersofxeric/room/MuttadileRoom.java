...
public final class MuttadileRoom extends RaidArea implements CycleProcessPlugin {
    ...
    public static final class MeatTreeWoodcutting extends Action {
        ...
        public boolean process() {
            if (ticks++ % 4 == 0) {
                player.setAnimation(axe.getDefinitions().getEmote());
            }
            return checkTree();
        }
        ...
        public int processWithDelay() {
            final int successRate = 20 + (int) Math.floor(player.getSkills().getLevel(Skills.WOODCUTTING) / 1.7F);
            ...
            player.getSkills().addXp(Skills.WOODCUTTING, 45);
            raid.addPoints(player, 2);
            tree.heal(-Utils.random(1, player.getSkills().getLevelForXp(Skills.WOODCUTTING)));
