...
public final class DeathlyRoom extends RaidArea {
    ...
    public void loadRoom() {
        final int amount = raid.getOriginalPlayers().size();
        ...
        for (int i = 0; i < magersAmount; i++) {
            npcs.add((DeathlyNPC) new DeathlyNPC(raid, this, 7560, getLocation(mages[Utils.random(mages.length - 1)])).spawn());
        }
        for (int i = 0; i < rangersAmount; i++) {
            npcs.add((DeathlyNPC) new DeathlyNPC(raid, this, 7559, getLocation(rangers[Utils.random(rangers.length - 1)])).spawn());
        }
        ...
        this.requirement = Math.min(getLowestLevel(Skills.AGILITY) + Utils.random(2), getHighestLevel(Skills.AGILITY));
    }
