 ...
 */
public abstract class RaidArea extends DynamicArea implements RoomController, DropPlugin, DeathPlugin, HitProcessPlugin, CannonRestrictionPlugin, TeleportPlugin, LogoutPlugin, LayableObjectPlugin {
    ...
    public void refreshStorageUnits(final int storageUnitId) {
        ...
        if (existingObject == null || !existingObject.getName().toLowerCase().contains("storage unit")) {
            return;
        }
    ...
    public Location getObjectLocation(final Location tile, final int sizeX, final int sizeY, final int rotation) {
        ...
        switch (rotation) {
            case 0:
                return t;
            case 1:
                return new Location(t.getX(), t.getY() - sx, t.getPlane());
            case 2:
                return new Location(t.getX() - sx, t.getY() - sy, t.getPlane());
            default:
                return new Location(t.getX() - sy, t.getY(), t.getPlane());
        }
    }
    ...
    public void enter(final Player player) {
        ...
        if (!originalPlayers.isEmpty() && !originalPlayers.contains(player.getUsername()) && !player.getPrivilege().eligibleTo(Privilege.ADMINISTRATOR)) {
            player.lock(1);
     ...
     */
    private final void updateStage(@NotNull final Player player) {
        ...
        final RaidArea room = type == null ? this : Utils.findMatching(raid.getMap().getRaidChunks(), chunk -> chunk.getToPlane() == plane && (chunk.getType() == RaidRoom.FLOOR_START_UPSTAIRS || chunk.getType() == null));
        if (room == null) {
            return;
        }
    ...
    public void leave(final Player player, boolean logout) {
        ...
        if (logout || !(nextArea instanceof RaidArea)) {
            ...
            player.setProtectionDelay(System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(10));
        ...
        if (this.leaveTime == 0 && !logout && nextArea instanceof RaidArea && (!player.getPrivilege().eligibleTo(Privilege.ADMINISTRATOR) || Constants.WORLD_PROFILE.isDevelopment())) {
            if (raid == null) {
                return;
            }
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (ticks == 1) {
                    ...
                    final Object killingBlow = player.getTemporaryAttributes().get("killing blow hit");
                    final String source = killingBlow instanceof NPC ? ((NPC) killingBlow).getDefinitions().getName() : "Unknown source";
                    builder.append(player.getUsername()).append(": Death at ").append(type == null ? "Great Olm" : type.getFormattedName()).append(" room by ").append(source).append(", losing ");
                    if (points >= 8400) {
                        builder.append((int) (points * 0.4F)).append(" personal points.");
                    } else {
                        builder.append((totalPoints - (int) (totalPoints * 0.9F))).append(" group points.");
                    }
                    raid.recordDeath(builder.toString());
                    player.reset();
                    player.setProtectionDelay(System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(5));
    ...
    @Override
    public void hit(final Player source, final Entity target, final Hit hit, final float modifier) {
     ...
     * The bound tile to which the player is moved to when they climb up/down from the "attached" room on another height level.
     */
    public void setBoundTile(final Location boundTile) {
     ...
     * The time in game ticks when the room was left, does not count if person leaves it the same way they entered. They must progress to the next room for it to count.
     */
    public void setLeaveTime(final long leaveTime) {
    ...
    public int getRemainingCombatPoints() {
        return this.remainingCombatPoints;
    }

    /**
     * The remaining combat points that can be granted within this raid room. Each room has a cap of points to avoid farming high tier rewards.
     */
    public void setRemainingCombatPoints(final int remainingCombatPoints) {
