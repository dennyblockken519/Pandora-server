...
public final class RaidFarming {
    ...
    private enum PatchType {
        GOLPAR(29998, 20906, 20904, 27, 4, 10),
        BUCHU(29999, 20909, 20907, 39, 6, 15),
        NOXIFER(29997, 20903, 20901, 55, 12, 30);
        ...
        PatchType(final int objectId, final int seedId, final int produceId, final int level, final int plantingXp, final int harvestingXp) {
    ...
    private static final class HerbHarvestAction extends Action {
        ...
        public boolean start() {
            if (player.getSkills().getLevel(Skills.FARMING) < patch.type.level) {
        ...
        public int processWithDelay() {
            ...
            player.getSkills().addXp(Skills.FARMING, patch.type.harvestingXp);
            player.getInventory().addOrDrop(new Item(patch.type.produceId, 1));
            final int req = patch.type.level;
            final double chance = Math.min(0.95, 0.65 + ((player.getSkills().getLevel(Skills.FARMING) - req) / 250.0F));
    ...
    private static final class HerbClearAction extends Action {
        ...
        public boolean start() {
            ...
            if (player.getSkills().getLevel(Skills.FARMING) < type.level) {
    ...
    private static final class HerbPlantAction extends Action {
        ...
        public int processWithDelay() {
            ...
            player.getSkills().addXp(Skills.FARMING, type.plantingXp);
