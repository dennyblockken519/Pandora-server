...
public final class LargeMuttadile extends RaidNPC<MuttadileRoom> implements CombatScript {
    ...
    private void iteratePath(@NotNull final Entity target, final int delay) {
        WorldTasksManager.schedule(new TickTask() {
            ...
            public void run() {
                ...
                final List<Location> path = Utils.calculateLine(current.getX(), current.getY(), target.getLocation().getX(), target.getLocation().getY(), current.getPlane());
    ...
    public void setUnleashed(final boolean unleashed) {
