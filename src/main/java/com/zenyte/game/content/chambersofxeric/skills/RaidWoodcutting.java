...
public final class RaidWoodcutting extends Action {
    ...
    public boolean process() {
        if (ticks++ % 4 == 0) player.setAnimation(hatchet.getDefinitions().getEmote());
        return tree.exists();
    }
    ...
    public int processWithDelay() {
        if (!success()) {
            return this.hatchet.getDefinitions().getCutTime();
        }
        addLog(player);
        if (Utils.random(7) == 0) {
    ...
    private void addLog(final Player player) {
        player.getRaid().ifPresent(raid -> {
            ...
            player.getInventory().addItem(20799, amount.intValue());
    ...
    private int getMaximumAmountKindling(@NotNull final Player player) {
        final int level = Math.max(player.getSkills().getLevel(Skills.WOODCUTTING), player.getSkills().getLevelForXp(Skills.WOODCUTTING));
        int max = 0;
