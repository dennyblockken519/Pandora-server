...
public final class LeftClaw extends GreatOlmClaw {
    ...
    boolean shouldClench() {
        ...
        clenchDamage.removeIf(hit -> {
            final boolean removed = hit.getScheduleTime() + TimeUnit.TICKS.toMillis(10) < System.currentTimeMillis();
            if (!removed) {
        ...
        final int threshold = room.getOlm().everyone(GreatOlm.ENTIRE_CHAMBER).size() * 20;
    ...
    public int getClenchTicks() {
        return this.clenchTicks;
    }

    /**
     * Defines how long the claw will remain clenched for.
     */
    public void setClenchTicks(final int clenchTicks) {
