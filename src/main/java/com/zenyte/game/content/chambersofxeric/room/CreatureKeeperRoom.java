...
public final class CreatureKeeperRoom extends RaidArea implements CycleProcessPlugin {
    ...
    public static void onServerLaunch(final ServerLaunchEvent event) {
        final ObjectArrayList<WorldObject> firstRoomList = new ObjectArrayList<>(64);
        final ObjectArrayList<WorldObject> secondRoomList = new ObjectArrayList<>(64);
        final ObjectArrayList<WorldObject> thirdRoomList = new ObjectArrayList<>(64);
    ...
    public void deposit(@NotNull final Player player, @NotNull final WorldObject object) {
        ...
        if (!player.getInventory().containsItem(20885, 1)) {
        ...
        final int amount = player.getInventory().getAmountOf(20885);
        trough.stage += amount;
        player.getInventory().deleteItem(20885, amount);
        player.sendMessage("You deposit some cavern grubs in the trough.");
        final int points = Math.min(pointsCap, 100 * amount);
        pointsCap -= points;
    ...
    public void loadRoom() {
        ...
        scavenger = new CorruptedScavenger(raid, this, 7602, getLocation(scavengerPositions[index][getRotation()]), Direction.mainDirections[(index + getRotation()) & 3]);
        scavenger.spawn();
        for (int x = scavenger.getX(); x < scavenger.getX() + scavenger.getSize(); x++) {
            for (int y = scavenger.getY(); y < scavenger.getY() + scavenger.getSize(); y++) {
                World.getRegion(Location.getRegionId(x, y), true).addFlag(scavenger.getPlane(), x & 63, y & 63, Flags.FLOOR);
            }
        }
     ...
     */
    private void recalculateHayPosition() {
        ...
        for (int x = this.hay.getX() - 4; x < this.hay.getX() + 4; x++) {
            for (int y = this.hay.getY() - 4; y < this.hay.getY() + 4; y++) {
                final Region region = World.getRegion(Location.getRegionId(x, y), true);
                final WorldObject[] objects = region.getObjects(getToPlane(), x & 63, y & 63);
                final WorldObject matchingObject = Utils.findMatching(objects, obj -> obj != null && (obj.getId() == 3471 || obj.getId() == 3472));
    ...
    public void setPointsCap(final int pointsCap) {
    ...
    public static final class Trough extends WorldObject {
        ...
        public boolean equals(@Nullable final Object o) {
            ...
            if (!(o instanceof Trough other)) return false;
    ...
    private static final class Chest extends WorldObject {
        ...
        public boolean equals(@Nullable final Object o) {
            if (o == this) return true;
            if (!(o instanceof Chest other)) return false;
            if (!other.canEqual(this)) return false;
            if (!super.equals(o)) return false;
    ...
    private static final class ChestLootingAction extends Action {
        ...
        public int processWithDelay() {
            ...
            final int success = (int) (lockpick ? (49.0F + (player.getSkills().getLevel(Skills.THIEVING) / 4.95F)) : (35.0F + (player.getSkills().getLevel(Skills.THIEVING) / 7.0F)));
            if (Utils.random(100) <= success) {
                ...
                if (type < 10) {
                    ...
                    chest.setId(29745);
                    final int thievingLevel = player.getSkills().getLevelForXp(Skills.THIEVING);
                    ...
                    player.getSkills().addXp(Skills.THIEVING, intAmount == 1 ? 40 : intAmount == 2 ? 60 : 73);
