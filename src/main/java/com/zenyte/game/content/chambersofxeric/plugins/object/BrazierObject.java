...
public class BrazierObject implements ObjectAction {
    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        player.getRaid().ifPresent(raid -> raid.ifInRoom(player, IceDemonRoom.class, room -> {
            final int chance = player.getSkills().getLevel(Skills.FIREMAKING) / 6;
    ...
    private void light(@NotNull final Player player, @NotNull final WorldObject object) {
        player.getRaid().ifPresent(raid -> raid.ifInRoom(player, IceDemonRoom.class, room -> {
            if (object.getId() == 29747) {
                ...
                final IcefiendNPC icefiend = Utils.findMatching(room.getIcefiends(), i -> i.getLocation().withinDistance(object, 1));
                final int amount = player.getInventory().getAmountOf(20799);
                final IceDemonRoom.Brazier brazier = new IceDemonRoom.Brazier(room, amount * 5, icefiend, 29748, object.getType(), object.getRotation(), new Location(object));
                ...
                player.sendSound(new SoundEffect(2066));
                player.getSkills().addXp(Skills.FIREMAKING, amount * 5);
            ...
            final IceDemonRoom.Brazier brazier = Utils.findMatching(room.getBraziers(), b -> b == object);
            if (brazier == null) {
                return;
            }
            final int amount = player.getInventory().getAmountOf(20799);
