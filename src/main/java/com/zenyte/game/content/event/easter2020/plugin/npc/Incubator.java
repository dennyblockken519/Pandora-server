...
public class Incubator extends NPCPlugin {
    ...
    public enum State {
        BROKEN(15229, 15224, EasterConstants.INCUBATOR_VARBIT, 0),
        WITH_COG(15230, 15225, EasterConstants.INCUBATOR_VARBIT, 1),
        WITH_PISTONS(15231, 15226, EasterConstants.INCUBATOR_VARBIT, 2),
        WITH_CHIMNEY(15232, 15227, EasterConstants.INCUBATOR_VARBIT, 3),
        FULLY_FIXED(15233, 15228, EasterConstants.INCUBATOR_VARBIT, 4);
        ...
        static {
            Utils.populateMap(values, forId = new Int2ObjectOpenHashMap<>(values.length), State::getId);
            Utils.populateMap(values, forVarbitValue = new Int2ObjectOpenHashMap<>(values.length), State::getVarbitValue);
        ...
        State(final int id, final int baseId, final int varbitId, final int varbitValue) {
            this.id = id;
            this.baseId = baseId;
    ...
    public static class ItemOnIncubator implements ItemOnNPCAction {
        ...
        public void handleItemOnNPCAction(Player player, Item item, int slot, NPC npc) {
            ...
            if (item.getName().toLowerCase().contains("pipe")) {
                player.getDialogueManager().start(new PlainChat(player, "Doesn't look like that will fit anywhere on this section. I wonder if it goes on one of the other units."));
                return;
            }
