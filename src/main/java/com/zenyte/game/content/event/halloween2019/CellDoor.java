    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        ...
        if (option.equalsIgnoreCase("Pick-lock")) {
            if (!player.getInventory().containsItem(ItemId.LOCKPICK, 1)) {
                player.getDialogueManager().start(new PlainChat(player, "You're going to need something to pick the lock with."));
                return;
            }
            pickLock(player, object);
        }
        ...
        if (shilopsCell) {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    item(new Item(ItemId.LOCKPICK), "The lockpick broke. It doesn't seem like a lockpick is going to be enough for this door...");
                }
            });
            return;
        }
