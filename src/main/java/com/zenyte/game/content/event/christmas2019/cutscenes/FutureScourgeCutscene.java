...
public class FutureScourgeCutscene extends Cutscene {
    ...
    public FutureScourgeCutscene(final Player player, final ScourgeHouseInstance instance, final Runnable unfadeRunnable) {
        this.player = player;
    ...
    public void build() {
        ...
        addActions(34, new DialogueAction(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "Well, if you don't wish to speak, allow me. We've had the Ghost of Christmas Past and the Ghost of Christmas Present - that much leave the Ghost of Christmas Yet to Come. Am I correct?", Expression.HIGH_REV_MAD);
        ...
        addActions(321, new DialogueAction(new Dialogue(player) {
            ...
            public void buildDialogue() {
                ...
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "I'll free that oa- I'll free Santa. I'll return the food!", Expression.HIGH_REV_MAD);
        ...
        addActions(330, new DialogueAction(new Dialogue(player) {
            ...
            public void buildDialogue() {
                ...
                npcWithId(ChristmasConstants.PERSONAL_SNOW_IMP, impName, "Let's get back to the feast, matey. We need to make sure Ol' Nick is safe!", Expression.HIGH_REV_NORMAL);
            }
        }));
    ...
    private void executeOptions(final int startTime, final Emote[] options, final Emote correctOption, final int jumpStage) {
        overrideActions(startTime, new DialogueAction(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                final ObjectArrayList<Dialogue.DialogueOption> optionsList = new ObjectArrayList<>();
        ...
        for (int i = 0; i < 3; i++) {
            ...
            final int duration = options[i].getAnimation().getDuration() / 600;
    ...
    private final String format(@NotNull final Emote emote) {
        return Utils.formatString(emote.name()) + ".";
    }
