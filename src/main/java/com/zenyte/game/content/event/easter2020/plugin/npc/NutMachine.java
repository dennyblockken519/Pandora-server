        ...
        bind("Look-at", new OptionHandler() {
            ...
            public void handle(Player player, NPC npc) {
                if (!SplittingHeirs.progressedAtLeast(player, Stage.RETRAIN_SQUIRRELS)) {
                    player.getDialogueManager().start(new PlayerChat(player, "I should speak with the Easter Bunny's son to see what to do next."));
                    return;
                }
                if (SplittingHeirs.getProgress(player) == Stage.RETRAIN_SQUIRRELS) {
                    player.getDialogueManager().start(new Dialogue(player, EasterConstants.BIG_BEN) {
                        ...
                        public void buildDialogue() {
                            ...
                            final String goldString = Utils.format(amountOfGold);
                            player("Hello there, cute, fluffly, little squirrel.");
                            npc("Who d'you fink yer talkin' to? I 'appen ta be Big Ben, no' some stoopid, fluffy, cotton wool-headed park animal. I's intelligent, I's knows what I's wants. And I's knows how ta gets it.", Expression.EASTER_BUNNY_NORMAL);
                            ...
                            npc("Stop righ' there, 'uman. If you's goin' ta call us rats wiv fluffy tails or vermin, jus' bear in mind tha' we all be owners of teef strong enuff ta crack a nut. And yes, I am in charge. Wha's it matter ta you?", Expression.EASTER_BUNNY_NORMAL);
                            player("Right...yes... Right you are, Mr Big Ben. I'm " + player.getName() + ", and I've come to you on behalf of the Easter Bunny.");
                            npc("Ahh, yes, da boss bunny.", Expression.EASTER_BUNNY_NORMAL);
                            player("Well, he'd like me to get the factory working again. That includes you going back to work.");
                            ...
                            options(TITLE, new DialogueOption("Hmm, okay, but only 40% of them.", key(FOURTY)), new DialogueOption("Hmm, okay, but only 20% of them.", key(TWENTY)), new DialogueOption("Hmm, okay, but only 5% of them.", key(FIVE)), new DialogueOption("Bye then.", this::finish));
                            {
                                ...
                                npc("Are you 'avin' a larf? If we ate tha' many, old EB would be outta business within a week! We may be greedy bu' we ain't stoopid.", Expression.EASTER_BUNNY_NORMAL);
                            ...
                            }
                            {
                                ...
                                npc("Well, one: don ever become a comedian, your yolks leave much ta be desired. Two: we...kind of forgot how to sort tha nuts, so you'll have to teach us.", Expression.EASTER_BUNNY_NORMAL);
                                player("Oh. As you seem to think I'm the eggs-pert. I guess I'll have to teach you, then.");
