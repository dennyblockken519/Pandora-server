...
public class FrozenGuestPlugin implements ObjectAction, ItemOnObjectAction {
    ...
    private final void throwWater(@NotNull final Player player, @NotNull final WorldObject object, @NotNull final OptionalInt bucketSlot) {
        final FrozenGuest guest = Utils.findMatching(FrozenGuest.getValues(), g -> g.getBaseObject() == object.getId());
        ...
        ChristmasConstants.refreshAllVarbits(player);
        ...
        for (int i = chars.length - 1; i >= 0; i--) {
            ...
            final FrozenGuest respectiveGuest = Utils.findMatching(FrozenGuest.getValues(), g -> g.getConstant() == character);
        ...
        WorldTasksManager.schedule(() -> {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    ...
                    npcWithId(ChristmasConstants.PERSONAL_SNOW_IMP, impName, "We's been wasting our time digging up Scourge's past. Past is way too long ago. Everyone knows misers have rubbish memory! This weeping boy, he's from presents.", Expression.HIGH_REV_NORMAL);
                    player("Bu-");
                    npcWithId(ChristmasConstants.PERSONAL_SNOW_IMP, impName, "If there is anyone who can melt the frozen heartstrings of that crankerous ol' miser, it's this angelic boy. We can use this boy to show Scourge how he's hurting people in the presents!", Expression.HIGH_REV_NORMAL);
                    ...
                    npcWithId(ChristmasConstants.PERSONAL_SNOW_IMP, impName, "Nah, he's perfect, mate! Let's pay Scourge a visit.", Expression.HIGH_REV_NORMAL).executeAction(() -> {
