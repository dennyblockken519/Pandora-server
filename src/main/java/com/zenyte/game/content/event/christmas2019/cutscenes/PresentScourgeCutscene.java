...
public class PresentScourgeCutscene extends Cutscene {
    ...
    public PresentScourgeCutscene(final Player player, final ScourgeHouseInstance instance, final Runnable unfadeRunnable) {
        this.player = player;
    ...
    public void build() {
        ...
        addActions(28, new CameraPositionAction(player, instance.getLocation(2475 + 1, 5404 + 1, 1), 500, 255, 100), new CameraLookAction(player, instance.getLocation(2465, 5396, 1), 0, 255, 100), new DialogueAction(new Dialogue(player) {
            ...
            public void buildDialogue() {
                ...
                npcWithId(ChristmasConstants.PERSONAL_SNOW_IMP, impName, "No way, dude! Say you is the 'Ghost of Christmas Present' - that'll scare 'im.", Expression.HIGH_REV_NORMAL).executeAction(() -> {
                    turnStraight();
                    imp.setInvalidAnimation(new Animation(15028));
                });
                player("I am the Ghost of Christmas Present, Scourge, and I'm here to show you the pain you still cause.");
                ...
                npcWithId(ChristmasConstants.PERSONAL_SNOW_IMP, impName, "We know someone who you've really hurt, Scourge. This is gonna make you feel sooooo bad.", Expression.HIGH_REV_NORMAL);
        ...
        addActions(44, this::turnRight, new DialogueAction(new Dialogue(player) {
            ...
            public void buildDialogue() {
                ...
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "You bring this spoilt little brat before me as proof of my misdeeds? You've only confirmed the righteousness of my actions.", Expression.HIGH_REV_MAD);
        ...
        addActions(47, new DialogueAction(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "You're lucky that your attempts have been so feeble they've bordered on the amusing, but I grow tired of your antics. Come back here again, 'ghost', and you shall not find me so merciful.", Expression.HIGH_REV_MAD).executeAction(() -> {
    ...
    private final void jumpToOptions() {
        overrideActions(31, new DialogueAction(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                final ObjectArrayList<Dialogue.DialogueOption> optionsList = new ObjectArrayList<>(options.size());
