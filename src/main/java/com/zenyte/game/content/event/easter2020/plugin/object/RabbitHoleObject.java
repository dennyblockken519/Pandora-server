...
public class RabbitHoleObject implements ObjectAction {
    ...
    private void transformToHuman(final Player player, final Runnable then) {
        ...
        player.lock(anim.getDuration() / 600);
        player.setAnimation(anim);
        if (then != null) {
            WorldTasksManager.schedule(then::run, anim.getDuration() / 600);
        }
    }
    ...
    private void enterBunnyTunnels(final Player player) {
        if (!player.getInventory().getContainer().isEmpty() || !player.getEquipment().getContainer().isEmpty()) {
            player.sendMessage("Your items won't fit through the rabbit hole - you need to bank them before going inside.");
            return;
        }
    ...
    private void exitToSurface(final Player player) {
        transformToHuman(player, () -> player.getDialogueManager().start(new PlainChat(player, "The Easter Bunny's magic fades, crumbling away the items you<br>collected, and changing you back to your usual self.")));
