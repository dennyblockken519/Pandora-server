...
public class PastScourgeCutsceneP2 extends Cutscene {
    ...
    public PastScourgeCutsceneP2(final Player player, final LandOfSnowInstance instance, final Runnable unfadeRunnable) {
        this.player = player;
    ...
    public void build() {
        ...
        addActions(9, new DialogueAction(new Dialogue(player) {
            ...
            public void buildDialogue() {
                npcWithId(ChristmasConstants.PERSONAL_SNOW_IMP, impName, "What's the big deal? They all look fine. A bit bored, mebbe.", Expression.HIGH_REV_NORMAL);
            }
        }));
        addActions(11, () -> {
            //perform animation
        ...
        for (int i = 0; i < length; i++) {
            ...
            addActions(16 + (i * 2), () -> {
                final FrozenGuest guest = Utils.findMatching(FrozenGuest.getValues(), g -> g.getConstant() == character);
