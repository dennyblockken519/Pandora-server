...
public class PersonalSnowImp extends NPCPlugin {
        ...
        bind("Talk-to", (player, npc) -> {
            ...
            player.getDialogueManager().start(new Dialogue(player, npc) {
                ...
                public void buildDialogue() {
                    ...
                    npc(impName, "We's the Queen of Snow's servants. We's were helping her prepare a feastie for all the people of " + GameConstants.SERVER_NAME + " when something bad happened.", Expression.HIGH_REV_NORMAL);
                    player("What happened?", Expression.ANXIOUS);
                    npc(impName, "Ower grumpy, nasty neighbour, Ezeneser Scourges, stole all the food we'd made for the feastie. Ol' Saint Nick tried to stop 'im, but Scourges overpowered 'im.", Expression.HIGH_REV_MAD);
                    ...
                    options(TITLE, options.toArray(new DialogueOption[0]));
                    {
                        ...
                        npc(impName, "Scourges is the foulest, most greedy and des-pick-able ooman about. Every time he leaves that ol' house of 'is he does something nasty.", Expression.HIGH_REV_NORMAL);
                        player("And did you say he overpowered Santa?");
                        npc(impName, "Scourges is a dark wizard. When Nick tried to stop 'im, Scourges bound him in chains with a snap o' his digits.", Expression.HIGH_REV_NORMAL).executeAction(() -> {
                            if (!AChristmasWarble.progressedAtLeast(player, AChristmasWarble.ChristmasWarbleProgress.SPOKEN_TO_PERSONAL_IMP)) {
                        ...
                        options(TITLE, new DialogueOption("Count me in!", key(COUNT_ME_IN)), new DialogueOption("Sorry, I don't have time right now.", this::finish));
                        {
                            ...
                            npc(impName, "Well, you ain't the brains, 'n you certainly awn't the looks. You ain't really the brawn neither - look at them skinny arms.", Expression.HIGH_REV_NORMAL);
    ...
    public enum Imp {
        ONE(1, 15009, ChristmasConstants.SNOW_IMP_VAR1),
        TWO(2, 15010, ChristmasConstants.SNOW_IMP_VAR2),
        THREE(3, 15011, ChristmasConstants.SNOW_IMP_VAR3),
        FOUR(4, 15012, ChristmasConstants.SNOW_IMP_VAR4);
        ...
        static {
            Utils.populateMap(values, byId = new Int2ObjectOpenHashMap<>(values.length), Imp::getIndex);
            Utils.populateMap(values, byBaseId = new Int2ObjectOpenHashMap<>(values.length), Imp::getBaseId);
        }
        ...
        Imp(final int index, final int baseId, final int varbit) {
            this.index = index;
            this.baseId = baseId;
            this.varbit = varbit;
        }
