...
public class PastScourgeCutsceneP1 extends Cutscene {
    ...
    public PastScourgeCutsceneP1(final Player player, final ScourgeHouseInstance instance, final Runnable unfadeRunnable) {
        this.player = player;
    ...
    public void build() {
        ...
        addActions(29, new DialogueAction(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                options("Choose an option:", new DialogueOption("I'm a ghost!", key(10)), new DialogueOption("I'm a human in bedsheets!", key(20)), new DialogueOption("I'm a giant floating napkin!", key(30)));
                player(10, "I'm a ghost!").executeAction(() -> setImpFacingPlayer());
                npcWithId(ChristmasConstants.PERSONAL_SNOW_IMP, impName, "Psst, say you's the 'Ghost of Christmas Past'. It's spookier.", Expression.HIGH_REV_NORMAL);
                player("I am the Ghost of Christmas Past!").executeAction(() -> setImpFacingScrouge());
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "You're not a ghost, I can see where you've sewn that old bedsheet together.", Expression.HIGH_REV_MAD);
                player("Bedsheets? These are no bedsheets. Those stitches you see represent all the people I've, er...");
                player("Stitched up!");
                player("My punishment for my sins is that I must look like I've been sewn together, poorly, for the rest of eternity.");
                ...
                options("Choose an option:", new DialogueOption("I'm a ghost!", key(10)), new DialogueOption("I'm a giant floating napkin!", key(30)));
                player(30, "I'm a giant floating napkin!").executeAction(() -> setImpFacingPlayer());
                npcWithId(ChristmasConstants.PERSONAL_SNOW_IMP, impName, "oi, don't say that! You's a ghost remember.", Expression.HIGH_REV_NORMAL);
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "What was that? Stop mumbling, speak properly!", Expression.HIGH_REV_MAD).executeAction(() -> setImpFacingScrouge());
                options("Choose an option:", new DialogueOption("I'm a ghost!", key(10)), new DialogueOption("I'm a human in bedsheets!", key(20)));
                player(90, "These chains represent my heavy guilt.");
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "Pfft, I can't be bothered with self-analyzing ghosts. Begone!", Expression.HIGH_REV_NORMAL);
                player("Let me finish! I was going to say: 'These chains represent my heavy guilt...is what I would say if I were a self-analyzing ghost'. But I'm not. These chains represent...");
                ...
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "You're telling me those chains are of no significance at all?", Expression.HIGH_REV_NORMAL);
                ...
                player("Warn you that if you don't change your ways you will be doomed to live like me for the rest of eternity!");
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "How dare you! I've done nothing wrong, nothing I tell you!", Expression.HIGH_REV_MAD);
                ...
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "I hardly think I'll be doomed to spend eternity in chains because I crashed that sappy little party.", Expression.HIGH_REV_MAD);
        ...
        addActions(41, new DialogueAction(new Dialogue(player) {
            ...
            public void buildDialogue() {
                ...
                npcWithId(ChristmasConstants.PAJAMAS_SCOURGE_NPC_ID, "'Nuthin\"? Your friends at the feast won't think it's 'nuthin\".", Expression.HIGH_REV_MAD).executeAction(() -> setImpFacingPlayer());
