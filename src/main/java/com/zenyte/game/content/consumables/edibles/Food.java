...
public enum Food implements Edible {
    JANGERBERRIES(247, 0) {
        @Override
        public Boost[] boosts() {
            return new Boost[]{new Boost(Skills.ATTACK, 0, 2), new Boost(Skills.STRENGTH, 0, 1), new Restoration(Skills.PRAYER, 0, 1), new Debuff(Skills.DEFENCE, 0, 1)};
        }
    },
    SHRIMP(315, 3),
    ANCHOVIES(319, 1),
    ...
    KEBAB(1971, 0) {
        ...
        public void onConsumption(final Player player) {
            ...
            if (roll <= 66) {
                ...
                healAmount = (int) (player.getSkills().getLevelForXp(Skills.HITPOINTS) * 0.1);
            } else if (roll <= 87) {
                ...
                healAmount = 30;
                player.getSkills().boostSkill(Skills.ATTACK, Utils.random(1, 3));
                player.getSkills().boostSkill(Skills.STRENGTH, Utils.random(1, 3));
                player.getSkills().boostSkill(Skills.DEFENCE, Utils.random(1, 3));
            }
    ...
    COOKED_KARAMBWAN(3144, 18) {
        ...
        @Override
        public void onConsumption(final Player player) {
            player.getVariables().setKarambwanDelay(3);
    ...
    WHITE_TREE_FRUIT(6469, 3) {
        ...
        public void onConsumption(final Player player) {
            if (player.getVariables().getRunEnergy() < 100) {
                player.getVariables().setRunEnergy(Math.min(100, player.getVariables().getRunEnergy() + Utils.random(5, 10)));
            }
        }
    },
    BAKED_POTATO(6701, 4),
    ...
    GARDEN_PIE(7178, 6, new Item(7180)) {
        @Override
        public Boost[] boosts() {
            return new Boost[]{new Boost(Skills.FARMING, 0, 3)};
        }
    },
    ...
    FISH_PIE(7188, 6, new Item(7190)) {
        @Override
        public Boost[] boosts() {
            return new Boost[]{new Boost(Skills.FISHING, 0, 3)};
        }
    },
    ...
    ADMIRAL_PIE(7198, 8, new Item(7200)) {
        @Override
        public Boost[] boosts() {
            return new Boost[]{new Boost(Skills.FISHING, 0, 5)};
        }
    },
    ...
    WILD_PIE(7208, 11, new Item(7210)) {
        @Override
        public Boost[] boosts() {
            return new Boost[]{new Boost(Skills.SLAYER, 0, 5), new Boost(Skills.RANGED, 0, 4)};
        }
    },
    ...
    SUMMER_PIE(7218, 11, new Item(7220)) {
        @Override
        public Boost[] boosts() {
            return new Boost[]{new Boost(Skills.AGILITY, 0, 5)};
        }

        @Override
        public void onConsumption(final Player player) {
            if (player.getVariables().getRunEnergy() < 100) {
                player.getVariables().setRunEnergy(Math.min(100, player.getVariables().getRunEnergy() + 10));
            }
        }
    },
    ...
    DRAGONFRUIT_PIE(ItemId.DRAGONFRUIT_PIE, 20, new Item(ItemId.HALF_A_DRAGONFRUIT_PIE)) {
        @Override
        public Boost[] boosts() {
            return new Boost[]{new Boost(Skills.FLETCHING, 0, 4)};
        }
    },
    ...
    LOACH(10970, 3) {
        @Override
        public Boost[] boosts() {
            return new Boost[]{new Boost(Skills.COOKING, 0, 1)};
        }
    },
    EEL_SUSHI(10971, 10),
    ...
    ANGLERFISH(13441, 0) {
        ...
        public int healedAmount(final Player player) {
            ...
            return (int) Math.floor(hitpoints / 10.0F) + c;
        }

        @Override
        public void heal(final Player player) {
            final int heal = healedAmount(player);
            final int hp = player.getSkills().getLevelForXp(Skills.HITPOINTS);
            final int currentHealth = player.getSkills().getLevel(Skills.HITPOINTS);
            final int boost = hp < 10 ? 0 : hp < 20 ? 3 : hp < 25 ? 4 : hp < 30 ? 6 : hp < 40 ? 7 : hp < 50 ? 8 : hp < 60 ? 11 : hp < 70 ? 12 : hp < 75 ? 13 : hp < 80 ? 15 : hp < 90 ? 16 : hp < 92 ? 17 : 22;
        ...
        public String endMessage(Player player) {
            final int hp = player.getSkills().getLevelForXp(Skills.HITPOINTS);
            final int boost = hp < 10 ? 0 : hp < 20 ? 3 : hp < 25 ? 4 : hp < 30 ? 6 : hp < 40 ? 7 : hp < 50 ? 8 : hp < 60 ? 11 : hp < 70 ? 12 : hp < 75 ? 13 : hp < 80 ? 15 : hp < 90 ? 16 : hp < 92 ? 17 : 22;
            if (player.getHitpoints() >= player.getMaxHitpoints() + boost) return null;
    ...
    BOTANICAL_PIE(19662, 7, new Item(19659)) {
        @Override
        public Boost[] boosts() {
            return new Boost[]{new Boost(Skills.HERBLORE, 0, 4)};
        }
    },
    ...
    MUSHROOM_PIE(21690, 8, new Item(21687)) {
        @Override
        public Boost[] boosts() {
            return new Boost[]{new Boost(Skills.CRAFTING, 0, 4)};
        }
    },
