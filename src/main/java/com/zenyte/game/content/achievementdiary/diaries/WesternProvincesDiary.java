...
public enum WesternProvincesDiary implements Diary {
    ...
    SCORE_GNOMEBALL_MATCH_GOAL(EASY, true, "Score a goal in a Gnomeball match."),
    CLAIM_CHOMPY_BIRD_HAT(EASY, NO_PREDICATE, "Claim any Chompy bird hat from Rantz."),  //done
    TELEPORT_TO_PEST_CONTROL(EASY, NO_PREDICATE, "Teleport to Pest Control using the Minigame teleports."), //done
    ...
    TAKE_GRAND_TREE_AGILITY_SHORTCUT(MEDIUM, NO_PREDICATE, "Take the agility shortcut from the Grand Tree to Otto's", "Grotto."),
    TRAVEL_TO_GNOME_STRONGHOLD(MEDIUM, NO_PREDICATE, "Travel to the Gnome Stronghold by Spirit Tree."), //done
    TRAP_A_SPINED_LARUPIA(MEDIUM, NO_PREDICATE, "Trap a Spined Larupia."), //done
    FISH_BASS(MEDIUM, p -> p.inArea("Ape Atoll"), "Fish some Bass on Ape Atoll."), //done
    ...
    TRAVEL_TO_FELDIP_HILLS(MEDIUM, NO_PREDICATE, "Travel to the Feldip Hills by Gnome Glider."),
    CLAIM_CHOMPY_BIRD_HAT_125_KILLS(MEDIUM, NO_PREDICATE, "Claim a Chompy bird hat from Rantz after registering at", "least 70 kills."),  //done
    TRAVEL_TO_FELDIP_HILLS_BY_EAGLE(MEDIUM, true, "Travel from Eagles' Peak to the Feldip Hills by Eagle."),
    MAKE_A_CHOCOLATE_BOMB(MEDIUM, true, "Make a Chocolate Bomb at the Grand Tree."),
    COMPLETE_GNOME_RESTAURANT_DELIVERY(MEDIUM, true, "Complete a delivery for the Gnome Restaurant."),
    CREATE_CRYSTAL_SAW(MEDIUM, true, "Turn your small crystal seed into a Crystal saw."),
    MINE_GOLD_ORE(MEDIUM, true, "Mine some Gold ore underneath the Grand Tree."),
    KILL_AN_ELF(HARD, true, "Kill an Elf with a Crystal bow."),
    CATCH_AND_COOK_MONKFISH(HARD, true, "Catch and cook a Monkfish in Piscatoris."),
    COMPLETE_VETERAN_PEST_CONTROL_GAME(HARD, NO_PREDICATE, "Complete a Veteran game of Pest Control."), //done
    ...
    BUILD_ISAFDAR_PAINTING(HARD, true, "Build an Isafdar painting in your POH Quest hall."),
    KILL_ZULRAH(HARD, NO_PREDICATE, "Kill Zulrah."), //done
    TELEPORT_TO_APE_ATOLL(HARD, NO_PREDICATE, "Teleport to Ape Atoll."), //done
    ...
    PROTECT_MAGIC_TREE(ELITE, p -> p.inArea("Tree Gnome Stronghold"), "Have Prissy Scilla protect your Magic tree."),
    USE_ELVEN_OVERPASS_SHORTCUT(ELITE, true, "Use the Elven overpass advanced cliffside shortcut."),
    EQUIP_COMPLETE_VOID_SET(ELITE, true, "Equip any complete void set."),
    CLAIM_CHOMPY_BIRD_HAT_1000_KILLS(ELITE, NO_PREDICATE, "Claim a Chompy bird hat from Rantz after registering at", "least 300 kills."),  //done
    ...
    static {
        for (final Diary.DiaryComplexity type : DiaryComplexity.VALUES) {
    ...
    public DiaryChunk[] chunks() {
        return new DiaryChunk[]{new DiaryChunk(6327, 11, 4511), new DiaryChunk(6328, 13, 4512), new DiaryChunk(6329, 13, 4513), new DiaryChunk(6330, 7, 4514)};
    }
