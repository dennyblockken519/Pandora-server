...
public enum LumbridgeDiary implements Diary {
    ...
    GRAPPLE_RIVER_LUM(MEDIUM, true, "Grapple across the River Lum."),
    PURCHASE_AN_UPGRADED_AVA(MEDIUM, true, "Purchase an upgraded device from Ava."),
    TRAVEL_TO_WIZARDS_TOWER(MEDIUM, NO_PREDICATE, "Travel to the Wizards' Tower by Fairy ring."), //done
    CAST_LUMBRIDGE_TELEPORT(MEDIUM, NO_PREDICATE, "Cast the teleport to Lumbridge spell."), //done
    ...
    PICKPOCKET_MASTER_GARDENER(MEDIUM, NO_PREDICATE, "Pickpocket Martin the Master Gardener."),
    GET_SLAYER_TASK_FROM_CHAELDAR(MEDIUM, NO_PREDICATE, "Get a slayer task from Chaeldar."), //done
    ...
    SQUEEZE_PAST_JUTTING_WALL(HARD, NO_PREDICATE, "Squeeze past the jutting wall on your way to the cosmic", "altar."),
    CRAFT_COSMIC_RUNES(HARD, NO_PREDICATE, "Craft 56 Cosmic runes simultaneously."), //done
    TRAVEL_TO_EDGEVILLE_ON_CANOE(HARD, "Travel from Lumbridge to Edgeville on a Waka Canoe."), //done
    COLLECT_TEARS_OF_GUTHIX(HARD, true, "Collect at least 100 Tears of Guthix in one visit."),
    TAKE_TRAIN_TO_KELDAGRIM(HARD, true, "Take the train from Dorgesh-Kaan to Keldagrim."),
    PURCHASE_BARROWS_GLOVES(HARD, NO_PREDICATE, "Purchase some Barrows gloves from the Lumbridge bank", "chest."),
    PICK_BELLADONNA(HARD, NO_PREDICATE, "Pick some Belladonna from the farming patch at Draynor", "Manor."), //done
    LIGHT_MINING_HELMET(HARD, true, "Light your mining helmet in the Lumbridge castle", "basement."),
    RECHARGE_PRAYER(HARD, CLAN_WARS::inArea, "Recharge your prayer at Clan Wars with Smite activated."), //done
    ...
    //done
    STEAL_FROM_DORGESH_KAAN_CHEST(ELITE, true, "Steal from a Dorgesh-Kaan rich chest."),
    PICKPOCKET_MOVARIO(ELITE, true, "Pickpocket Movario on the Dorgesh-Kaan Agility course."),
    CHOP_MAGIC_LOGS(ELITE, MAGE_TRAINING_ARENA::inArea, "Chop some magic logs at the Mage Training Arena."), //done
    ...
    PERFORM_QUEST_CAPE_EMOTE(ELITE, true, WISE_OLD_MAN_HOUSE::inArea, "Perform the Quest cape emote in the Wise Old Man's", "house.");
    ...
    public DiaryChunk[] chunks() {
        return new DiaryChunk[]{new DiaryChunk(6311, 12, 4535), new DiaryChunk(6312, 12, 4536), new DiaryChunk(6313, 11, 4537), new DiaryChunk(6314, 6, 4538)};
    }
