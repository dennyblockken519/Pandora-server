...
public final class PestControlUtilities {
    ...
    private static void updateLander(final OptionalInt time, final Area... areas) {
        for (final Area area : areas) {
            if (!(area instanceof AbstractLanderArea lander)) {
                continue;
            }
    ...
    public static void updateLanderInformation(final Player player, final AbstractLanderArea area) {
        GameInterface.PEST_CONTROL_LANDER_OVERLAY.getPlugin().ifPresent(inter -> {
            ...
            if (area.getPlayers().size() >= MINIMUM_PLAYERS_LIMIT)
                dispatch.sendComponentText(overlay, inter.getComponent("Players ready"), "Players ready: " + area.getPlayers().size());
            else
                dispatch.sendComponentText(overlay, inter.getComponent("Players ready"), "<col=ff813d>Players ready: " + area.getPlayers().size() + " (" + (MINIMUM_PLAYERS_LIMIT - area.getPlayers().size()) + " needed)");
