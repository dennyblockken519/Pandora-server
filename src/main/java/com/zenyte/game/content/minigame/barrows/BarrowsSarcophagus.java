...
public class BarrowsSarcophagus implements ObjectAction {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        if (option.equals("Search")) {
            final BarrowsWight wight = Utils.findMatching(BarrowsWight.values, npc -> npc.getSarcophagusId() == object.getId());
                ...
                if (wight.equals(barrows.getHiddenWight())) {
                    player.getDialogueManager().start(new Dialogue(player) {
                        ...
                        public void buildDialogue() {
                            final ObjectArrayList<Dialogue.DialogueOption> options = new ObjectArrayList<>();
                            if (player.getMemberRank().eligibleTo(MemberRank.EMERALD_MEMBER) && barrows.isSkipTunnels()) {
                                options.add(new DialogueOption("Take me to the chest!", () -> {
                                    final Barrows barrows = player.getBarrows();
                            ...
                            options.add(new DialogueOption("Yes, I'm fearless.", () -> {
                                final Barrows barrows = player.getBarrows();
                                final BarrowsCorner corner = barrows.getCorner();
                                player.setLocation(corner.ladder.transform(0, -1, 0));
                                barrows.refreshDoors();
                            }));
                            options.add(new DialogueOption("No way, that looks scary!"));
                            options(options.size() == 3 ? "You've found two tunnels.<br>Would you like to enter either?" : "You've found a tunnel.<br>Would you like to enter it?", options.toArray(new DialogueOption[0]));
                        }
                    });
                    return;
                }
