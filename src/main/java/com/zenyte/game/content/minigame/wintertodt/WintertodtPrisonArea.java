 ...
 */
public class WintertodtPrisonArea extends WintertodtCampArea implements CannonRestrictionPlugin, LayableTrapRestrictionPlugin, DropPlugin, RandomEventRestrictionPlugin, LogoutPlugin, LoginPlugin, CycleProcessPlugin {
    ...
    private static HashSet<Location> getSubLocations(final Location middleLocation) {
        final HashSet<Location> subLocations = new HashSet<>(9);
    ...
    private static boolean canAOEInLocation(final HashSet<Location> locations) {
        for (Location location : locations) {
            ...
            if (!Utils.isSquareFree(location.getX(), location.getY(), 0, 1)) {
                return false;
            }
        }
        return true;
    ...
    private void randomColdDamage(final Player player) {
        ...
        if (Utils.random(0, chanceToHit) == 0) {
            ...
            final int randomHitAmount = ((16 - Wintertodt.getNumberOfWarmClothingWorn(player) - (2 * braziersLit)) * (player.getMaxHitpoints() + 1)) / player.getSkills().getLevelForXp(Skills.FIREMAKING);
