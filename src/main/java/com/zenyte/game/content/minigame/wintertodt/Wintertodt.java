...
public class Wintertodt {
    ...
    private static void finishRound() {
        for (final Player player : GlobalAreaManager.get("Wintertodt").getPlayers()) {
            ...
            if (points < 500) {
                ...
                final int rewardExp = player.getSkills().getLevelForXp(Skills.FIREMAKING) * 100;
                final Item crate = generateSupplyCrate(player, points);
                player.getSkills().addXp(Skills.FIREMAKING, rewardExp, false);
                player.sendMessage("You have gained " + Utils.format(rewardExp * player.getSkillingXPRate()) + " Firemaking XP.");
                ...
                player.sendMessage("Your subdued Wintertodt count is: " + Colour.RED.wrap(String.valueOf(kills)) + ".");
    ...
    static Item generateSupplyCrate(final Player player, final int points) {
        ...
        if (player.getMemberRank().eligibleTo(MemberRank.EMERALD_MEMBER)) {
    ...
    static int getBaseAOEDamageAmount(final Player player) {
        return ((10 - Wintertodt.getNumberOfWarmClothingWorn(player)) * player.getMaxHitpoints() + 1) / player.getSkills().getLevelForXp(Skills.FIREMAKING);
    }

    public static boolean isDisabled() {
        return Wintertodt.disabled;
    }
    ...
    enum Corner {
        SOUTH_WEST(new Pyromancer(new Location(1619, 3996), Direction.NORTH_EAST), new Brazier(1620, 3997)),
        NORTH_WEST(new Pyromancer(new Location(1619, 4018), Direction.SOUTH_EAST), new Brazier(1620, 4015)),
        NORTH_EAST(new Pyromancer(new Location(1641, 4018), Direction.SOUTH_WEST), new Brazier(1638, 4015)),
        SOUTH_EAST(new Pyromancer(new Location(1641, 3996), Direction.NORTH_WEST), new Brazier(1638, 3997));
        ...
        Corner(final Pyromancer pyromancer, final Brazier brazier) {
