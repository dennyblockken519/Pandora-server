 ...
 */
public final class PestControlInstance extends DynamicArea implements DeathPlugin, CycleProcessPlugin, LogoutPlugin, CannonRestrictionPlugin {
    ...
    int getActivity(final Player player) {
        return Math.max(0, mappedPlayers.get(player).getActivity());
    }
    ...
    public List<WorldObject> getRavagableObjects() {
        final ObjectArrayList<WorldObject> ravagableObjects = new ObjectArrayList<>();
    ...
    private void exportPlayer(final Player player, final boolean successful) {
        ...
        WorldTasksManager.schedule(() -> {
            if (successful) {
                ...
                if (rank.eligibleTo(MemberRank.ONYX_MEMBER)) {
                    points += 5;
                } else if (rank.eligibleTo(MemberRank.DRAGONSTONE_MEMBER)) {
                    points += 4;
                ...
                player.getInventory().addOrDrop(new Item(995, player.getSkills().getCombatLevel() * 150));
    ...
    private void spawn() {
        if (monsters.size() >= 150 || availablePortals.size() == 0) {
            return;
        }
