...
public class ChopBrumaRoot implements ObjectAction {
    ...
    private static final class ChopBrumaRootAction extends Action {
        ...
        private boolean success() {
            final int level = player.getSkills().getLevel(Skills.WOODCUTTING) - 1;
            return Math.min(Math.round(level * 0.4F) + 55, 90) > Utils.random(100);
        }

        private int chopDelay() {
            return Math.max(axe.getDefinitions().getCutTime(), AxeDefinitions.STEEL.getCutTime()) - 3;
        }
        ...
        public int processWithDelay() {
            ...
            player.getSkills().addXp(Skills.WOODCUTTING, player.getSkills().getLevelForXp(Skills.WOODCUTTING) * 0.3);
            return chopDelay();
        }
