...
public class CatapultInterface extends Interface {
     ...
     */
    private static int getModel(final int digit) {
        if (digit < 0 || digit > 9) {
    ...
    private void refreshDigits(@NotNull final Player player) {
        ...
        player.send(new IfSetModel(id, getComponent("First horizontal digit"), getModel(firstHorizontalDigit)));
        player.send(new IfSetModel(id, getComponent("Second horizontal digit"), getModel(secondHorizontalDigit)));
        player.send(new IfSetModel(id, getComponent("First vertical digit"), getModel(firstVerticalDigit)));
        player.send(new IfSetModel(id, getComponent("Second vertical digit"), getModel(secondVerticalDigit)));
    }
    ...
    private void refreshCross(@NotNull final Player player) {
        ...
        player.send(new IfSetPosition(id, getComponent("Cross"), startX + (horizontalClicks * multiplier), startY + (verticalClicks * multiplier)));
    }
