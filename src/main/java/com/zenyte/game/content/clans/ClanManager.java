...
public final class ClanManager implements ScheduledExternalizable {
    ...
    public static void join(@NotNull final Player player, @NotNull final String rq) {
        ...
        final String requestedName = rq.equalsIgnoreCase("help") ? GameConstants.SERVER_NAME : rq;
        final String name = Utils.formatUsername(requestedName);
        player.sendMessage("Attempting to join channel...");
        //Schedule on a task to properly introduce the "delay" that OSRS gets.
        WorldTasksManager.schedule(() -> {
            final Optional<ClanChannel> optional = ClanManager.getChannel(name);
     ...
     * @param force
     */
    public static void kick(@NotNull final Player player, final boolean inform, @NotNull final Player beingKicked, final boolean force) {
        if (player == null) {
            throw new NullPointerException("player is marked non-null but is null");
        }
        if (beingKicked == null) {
     ...
     */
    public static void message(@NotNull final Player player, @NotNull final ChatMessage message) {
        if (player == null) {
            throw new NullPointerException("player is marked non-null but is null");
        }
        if (message == null) {
    ...
    public static void refreshChannel(@NotNull final ClanChannel channel) {
        ...
        channel.loadOwner(owner -> {
            ...
            final ClanChannelBuilder builder = new ClanChannelFullBuilder(channel, owner).build();
    ...
    public static void refreshPartial(@NotNull final ClanChannel channel, final Player player, final boolean added, final boolean split) {
        if (channel == null) {
            throw new NullPointerException("channel is marked non-null but is null");
        }
        channel.loadOwner(owner -> {
            final Set<Player> members = channel.getMembers();
            members.removeIf(member -> member.isNulled() || member.isFinished());
            final ClanChannelBuilder builder = new ClanChannelMemberBuilder(player, owner, added, channel).build();
     ...
     */
    private static boolean canEnter(@NotNull final Player player, @NotNull final Player clanOwner, @NotNull final ClanChannel channel) {
        if (player == null) {
            throw new NullPointerException("player is marked non-null but is null");
        }
        if (channel == null) {
     ...
     */
    private static boolean canTalk(@NotNull final Player player, @NotNull final Player clanOwner, @NotNull final ClanChannel channel) {
        if (player == null) {
            throw new NullPointerException("player is marked non-null but is null");
        }
        if (channel == null) {
