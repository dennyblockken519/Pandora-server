 ...
 */
public class TearsOfGuthixCaveArea extends TearsOfGuthixArea implements CycleProcessPlugin, DeathPlugin, TeleportPlugin, EquipmentPlugin {
    /**
    ...
    /**
     * The temporary attribute which determines whether player is already being removed. If so, then no processing needs to take place.
     */
     ...
     */
    private static final List<GameInterface> TABS_INTERFACE_POS = Arrays.asList(GameInterface.COMBAT_TAB, GameInterface.SKILLS_TAB, GameInterface.JOURNAL_HEADER_TAB, GameInterface.SETTINGS, GameInterface.EMOTE_TAB, GameInterface.MUSIC_TAB, GameInterface.INVENTORY_TAB, GameInterface.EQUIPMENT_TAB, GameInterface.PRAYER_TAB_INTERFACE, GameInterface.SPELLBOOK, GameInterface.FRIEND_LIST_TAB, GameInterface.GAME_NOTICEBOARD, GameInterface.LOGOUT, GameInterface.CLAN_CHAT_TAB);
    ...
    public static void remove(@NotNull final Player player) {
        ...
        final int lastDistance = JunaEnterDialogue.INSIDE_CAVE_TILE.getTileDistance(JunaEnterDialogue.OUTSIDE_CAVE_TILE);
        ...
        player.addWalkSteps(JunaEnterDialogue.INSIDE_CAVE_TILE.getX(), JunaEnterDialogue.INSIDE_CAVE_TILE.getY(), firstDistance, false);
        ...
        WorldTasksManager.schedule(() -> {
            player.addWalkSteps(JunaEnterDialogue.OUTSIDE_CAVE_TILE.getX(), JunaEnterDialogue.OUTSIDE_CAVE_TILE.getY(), lastDistance, false);
        }, firstDistance + 1);
        WorldTasksManager.schedule(() -> {
        ...
        WorldTasksManager.schedule(() -> {
            if (!player.getLocation().equals(JunaEnterDialogue.OUTSIDE_CAVE_TILE)) {
                return;
            }
            final int lowestSkill = getLowestSkill(player);
            final double expPerTear = Math.min(60, Utils.linearInterpolate(1, 10, 20, 26.6, player.getSkills().getLevelForXp(lowestSkill)));
            final boolean eligibleForBonusExp = DiaryReward.EXPLORERS_RING3.eligibleFor(player);
            final double rewardedExp = expPerTear * tearAmount * (eligibleForBonusExp ? 1.1 : 1);
            player.getEquipment().deleteItem(new Item(ItemId.STONE_BOWL));
            player.getSkills().addXp(lowestSkill, rewardedExp);
            player.log(LogLevel.INFO, "Received " + rewardedExp + " experience from TOG for " + Skills.getSkillName(lowestSkill));
            ...
            if (eligibleForBonusExp) {
                player.sendMessage(Colour.RED.wrap("You are awarded an additional 10% experience for completing the hard Lumbridge achievement diary."));
            }
        }, firstDistance + lastDistance + 3);
    }

    /**
     * Returns the skill with lowest experience following the order of {@link Skills#SKILL_IDS}.
     */
    private static int getLowestSkill(@NotNull final Player player) {
        int lowestSkill = Skills.SKILL_IDS.getInt(0);
        for (final Integer skillId : Skills.SKILL_IDS) {
            final double experience = player.getSkills().getExperience(skillId);
            if (experience < player.getSkills().getExperience(lowestSkill) && (skillId != Skills.CONSTRUCTION || Constants.CONSTRUCTION)) {
                lowestSkill = skillId;
            }
        }
        return lowestSkill;
    ...
    public void leave(Player player, boolean logout) {
        ...
        for (final GameInterface position : TABS_INTERFACE_POS) {
            player.getInterfaceHandler().sendInterface(position);
        }
        if (logout || player.getLocation().getDistance(JunaEnterDialogue.OUTSIDE_CAVE_TILE) > 5) {
            player.getEquipment().deleteItem(new Item(ItemId.STONE_BOWL));
            player.getAppearance().resetRenderAnimation();
        }
