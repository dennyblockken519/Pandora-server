...
public final class StatRestorePotShare implements ItemSpell {
    ...
    public boolean spellEffect(final Player player, final Item item, final int slot) {
        ...
        if (!(consumable instanceof Potion potion) || !ArrayUtils.contains(VALID_POTIONS, consumable)) {
        ...
        final List<Player> characters = player.findCharacters(1, Player.class, p2 -> p2 != player && !p2.isDead() && p2.getBooleanSetting(Setting.ACCEPT_AID) && p2.getDuel() == null);
