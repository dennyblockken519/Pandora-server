...
public class PrayerManager {
    ...
    public void togglePrayer(int id, final boolean quickPrayers) {
        ...
        final boolean activating = player.getVarManager().getBitValue(prayer.getVarbit()) == 0;
        if (activating) {
    ...
    public double getRangedBoost(final int style) {
        ...
        if (activePrayers.containsKey(Prayer.SHARP_EYE)) {
            ...
            return style == Skills.ATTACK ? 1.2 : style == Skills.STRENGTH ? 1.23 : 1;
        }
        return 1;
    }
    ...
    public double getMagicBoost(final int style) {
        if (activePrayers.containsKey(Prayer.MYSTIC_WILL)) {
            ...
            return style == Skills.ATTACK ? 1.25 : style == Skills.DEFENCE ? 1.25 : 1;
        }
        return 1;
    }
    ...
    public void setQuickPrayer(final int id) {
        final Prayer prayer = Prayer.get(id);
        if (prayer == null || player.isDead() || !hasRequirements(prayer, false) || player.isLocked()) {
            if (prayer != null) {
                refreshQuickPrayers();
            }
            return;
    ...
    public void process() {
        //val blessing = player.getPerkManager().isValid(PerkWrapper.ITS_A_BLESSING);
        //This perk is too powerful to even consider as such, leaving it out.
        activePrayers.forEach((k, v) -> {
            if (v < (int) WorldThread.WORLD_CYCLE) {
                drain += k.getDrainRate();
            }
        });
        final int resistance = 60 + (player.getBonuses().getBonus(Bonuses.Bonus.PRAYER) * 2);
    ...
    public boolean hasRequirements(final Prayer prayer, final boolean checkPoints) {
        ...
        if (player.getSkills().getLevelForXp(Skills.PRAYER) < prayer.getLevel()) {
        ...
        if (prayer.equals(Prayer.CHIVALRY) && player.getSkills().getLevelForXp(Skills.DEFENCE) < 65) {
        ...
        if (prayer.equals(Prayer.PIETY) && player.getSkills().getLevelForXp(Skills.DEFENCE) < 70) {
        ...
        if ((prayer.equals(Prayer.RIGOUR) && (!settings.isRigour() || player.getSkills().getLevelForXp(Skills.DEFENCE) < 70)) || (prayer.equals(Prayer.AUGURY) && (!settings.isAugury() || player.getSkills().getLevelForXp(Skills.DEFENCE) < 70)) || (prayer.equals(Prayer.PRESERVE) && !settings.isPreserve())) {
    ...
    public void restorePrayerPoints(final int amount) {
        final int level = player.getSkills().getLevelForXp(Skills.PRAYER);
        final int points = getPrayerPoints();
        if (points >= level) {
            return;
        }
    ...
    public boolean isActive(final Prayer prayer) {
        return player.getVarManager().getBitValue(prayer.getVarbit()) == 1;
    }
    ...
    public void applyRedemptionEffect() {
        player.heal((int) (player.getSkills().getLevelForXp(Skills.PRAYER) * 0.25F));
        setPrayerPoints(0);
    ...
    public void applyRetributionEffect(final Entity source) {
        ...
        CharacterLoop.forEach(player.getLocation(), 1, Entity.class, p -> {
            if (p == player || p.isDead() || p instanceof Player && !((Player) p).isCanPvp() || (p instanceof NPC && !((NPC) p).isAttackable()))
                return;
    ...
    public int getQuickPrayerSettings() {
        return this.quickPrayerSettings;
    }

    public void setQuickPrayerSettings(final int quickPrayerSettings) {
