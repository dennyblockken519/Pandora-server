...
public enum GroupTeleport implements TeleportSpell, Teleport {
    TELE_GROUP_MOONCLAN(67, TeleportCollection.TELE_GROUP_MOONCLAN),
    TELE_GROUP_WATERBIRTH(72, TeleportCollection.TELE_GROUP_WATERBIRTH),
    TELE_GROUP_BARBARIAN(77, TeleportCollection.TELE_GROUP_BARBARIAN),
    TELE_GROUP_KHAZARD(79, TeleportCollection.TELE_GROUP_KHAZARD),
    TELE_GROUP_FISHING_GUILD(86, TeleportCollection.TELE_GROUP_FISHING_GUILD),
    TELE_GROUP_CATHERBY(93, TeleportCollection.TELE_GROUP_CATHERBY),
    TELE_GROUP_ICE_PLATEAU(99, TeleportCollection.TELE_GROUP_ICE_PLATEAU);
    ...
    private final void perform(final Player player) {
        ...
        player.findCharacters(3, Player.class, p -> !p.isLocked() && !p.isDead() && p.getBooleanSetting(Setting.ACCEPT_AID) && !p.getInterfaceHandler().containsInterface(InterfacePosition.CENTRAL) && !p.getInterfaceHandler().containsInterface(InterfacePosition.DIALOGUE) && p != player).forEach(p -> Teleother.request(player, p, teleport));
        teleport(player);
    }
