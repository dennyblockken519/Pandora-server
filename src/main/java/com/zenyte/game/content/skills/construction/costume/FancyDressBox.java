...
public final class FancyDressBox {
    @Expose
    private final Map<Integer, int[]> items = new HashMap<>(FancyDressData.VALUES.length);
    ...
    public void takeSet(final int slotId) {
        final Item item = FancyDressData.CONTAINER.get(slotId / 4);
        if (item == null) {
            return;
        }
        final FancyDressData set = FancyDressData.DISPLAY_MAP.get(item.getId());
        if (set == null) {
            return;
        }
    ...
    private void refresh() {
        final FancyDressData[] data = FancyDressData.VALUES;
        int firstHash = 0;
        int secondHash = 0;
        int count = 0;
        for (int i = 0; i < 40; i++) {
        ...
        player.getPacketDispatcher().sendClientScript(417, 100, firstHash, secondHash, "Fancy Dress Box");
    ...
    public void open() {
        ...
        player.getPacketDispatcher().sendUpdateItemContainer(100, FancyDressData.CONTAINER);
        refresh();
    }
