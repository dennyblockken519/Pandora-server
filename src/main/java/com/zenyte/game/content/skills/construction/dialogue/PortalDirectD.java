...
public final class PortalDirectD extends OptionsMenuD {
    ...
    public void handleClick(int slotId) {
        ...
        try {
            ...
            Furniture furn = null;
            if (data.getFurniture().getObjectId() == 13636 || data.getFurniture().ordinal() >= Furniture.TEAK_VARROCK_PORTAL.ordinal() && data.getFurniture().ordinal() <= Furniture.TEAK_KOUREND_PORTAL.ordinal())
                furn = Furniture.VALUES[Furniture.TEAK_VARROCK_PORTAL.ordinal() + slotId];
            else if (data.getFurniture().getObjectId() == 13637 || data.getFurniture().ordinal() >= Furniture.MAHOGANY_VARROCK_PORTAL.ordinal() && data.getFurniture().ordinal() <= Furniture.MAHOGANY_KOUREND_PORTAL.ordinal())
                furn = Furniture.VALUES[Furniture.MAHOGANY_VARROCK_PORTAL.ordinal() + slotId];
            else if (data.getFurniture().getObjectId() == 13638 || data.getFurniture().ordinal() >= Furniture.MARBLE_VARROCK_PORTAL.ordinal() && data.getFurniture().ordinal() <= Furniture.MARBLE_KOUREND_PORTAL.ordinal())
                furn = Furniture.VALUES[Furniture.MARBLE_VARROCK_PORTAL.ordinal() + slotId];
            if (furn == null) return;
            ...
            if (player.getSkills().getLevel(Skills.MAGIC) < portal.getTeleport().getLevel() || !player.getInventory().containsItems(runes)) {
