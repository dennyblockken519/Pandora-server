...
public enum ToyBoxData {
    ...
    public static final ItemContainer[] CONTAINERS = new ItemContainer[]{new ItemContainer(40, false), new ItemContainer(23, false)};
    public static final Map<Integer, ToyBoxData> MAP = new HashMap<>(VALUES.length * 5);
    public static final Map<Integer, ToyBoxData> DISPLAY_MAP = new HashMap<>(VALUES.length);

    static {
        for (int i = 0; i <= GRAVEDIGGER_OUTFIT.ordinal(); i++) CONTAINERS[0].add(new Item(VALUES[i].displayItem));
        ...
        for (ToyBoxData val : VALUES) {
            for (StorableSetPiece p : val.pieces) for (int i : p.getIds()) MAP.put(i, val);
