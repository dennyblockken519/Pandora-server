...
public class Farming {
    ...
    public static JsonDeserializer<FarmingSpot> deserializer() {
    ...
    private final Optional<FarmingSpot> getSpot(@NotNull final FarmingPatch patch) {
        final FarmingSpot spot = Utils.findMatching(spots, s -> patch.equals(s.getPatch()));
    ...
    public final Optional<FarmingPatch> getPatch(@NotNull final WorldObject object) {
        final FarmingPatch patch = Utils.findMatching(FarmingPatch.values, p -> ArrayUtils.contains(p.getIds(), object.getId()));
