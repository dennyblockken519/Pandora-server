...
public final class Bench implements ObjectInteraction {
    ...
    private static final Animation[] ANIMS = new Animation[]{
            new Animation(4089), new Animation(4091), new Animation(4093), new Animation(4095),
            new Animation(4097), new Animation(4099), new Animation(4101),
            new Animation(7285), new Animation(7287), new Animation(7289), new Animation(7291),
            new Animation(7293), new Animation(7295), new Animation(7297), new Animation(7299),
    };
    ...
    public Object[] getObjects() {
        return new Object[]{13300, 13301, 13302, 13303, 13304, 13305, 13306, 29270, 29271,
        ...
        public void stop() {
            ...
            WorldTasksManager.schedule(() -> {
                if (player.getLocation().getPositionHash() == object.getPositionHash()) {
                    player.addWalkSteps(face.getX() > player.getX() ? (player.getX() - 1) : face.getX() < player.getX() ? (player.getX() + 1) : player.getX(),
                            face.getY() > player.getY() ? (player.getY() - 1) : face.getY() < player.getY() ? (player.getY() + 1) : player.getY(), 1, false);
                }
