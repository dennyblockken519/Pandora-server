...
public class Sawmill extends Action {
    ...
    public Sawmill(final Plank plank, final int amount) {
        this.plank = plank;
        this.amount = amount;
    }
    ...
    private boolean check(final boolean message) {
        if (!player.getInventory().containsItem(plank.getBase())) {
            ...
            if (message) {
                player.getDialogueManager().start(new NPCChat(player, 5422, "You'll need to bring me some more logs."));
            }
            return false;
        }
        if (!player.getInventory().containsItem(new Item(995, plank.getCost()))) {
            player.getInterfaceHandler().closeInterface(InterfacePosition.CENTRAL);
            if (message) {
                player.getDialogueManager().start(new NPCChat(player, 5422, "Those planks cost " + plank.getCost() + ". You don't have enough<br>money for all of them."));
            }
            return false;
        }
        return true;
