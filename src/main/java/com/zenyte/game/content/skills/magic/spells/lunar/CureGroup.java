...
public final class CureGroup implements DefaultSpell {
    ...
    public boolean spellEffect(final Player player, final int optionId, final String option) {
        ...
        int count = 0;
        final List<Player> characters = player.findCharacters(1, Player.class, p2 -> p2 != player && !p2.isDead() && p2.getBooleanSetting(Setting.ACCEPT_AID) && p2.getDuel() == null && (p2.getToxins().isPoisoned() || p2.getToxins().isVenomed()));
