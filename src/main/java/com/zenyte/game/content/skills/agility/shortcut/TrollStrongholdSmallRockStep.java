...
public class TrollStrongholdSmallRockStep implements Shortcut {
    ...
    public void startSuccess(final Player player, final WorldObject object) {
        ...
        if (object.getId() == 3803 || object.getId() == 3804 || object.getId() == 3748) {
            if (object.withinDistance(2888, 3660, 3) || object.withinDistance(2856, 3611, 3) || object.withinDistance(2834, 3628, 3)) {
                ...
                forceMovement = new ForceMovement(destination, 60, Utils.getFaceDirection(destination.getX() - player.getX(), destination.getY() - player.getY()));
            } else {
                destination = object.transform(player.getX() > object.getX() ? -1 : 1, 0, 0);
                forceMovement = new ForceMovement(destination, 60, Utils.getFaceDirection(destination.getX() - player.getX(), destination.getY() - player.getY()));
            }
        } else if (object.getId() == 16523) {
    ...
    public int getLevel(final WorldObject object) {
        if (object.withinDistance(2856, 3611, 3)
                || object.withinDistance(2834, 3628, 3)
                || object.withinDistance(2821, 3635, 3)) {
            return 0;
        }
        return (object.getId() == 3803 || object.getId() == 3804) ? 43 : object.getId() == 3748 ? 44 : object.getId() == 16523 ? 44 :
                (object.getId() >= 26400 && object.getId() <= 26402) ? 60 : 0;
    }
