    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        if (option.equalsIgnoreCase("Worship")) {
            ...
            for (final Int2ObjectMap.Entry<Item> item : inventory.getContainer().getItems().int2ObjectEntrySet()) {
                if ((bonemeal = Utils.findMatching(BoneGrinding.Bonemeal.values, meal -> meal.getBonemeal() == item.getValue().getId())) != null) {
                    break;
                }
            }
            if (bonemeal == null) {
            ...
            player.getSkills().addXp(Skills.PRAYER, bonemeal.getBones().getXp() * 4.0F);
