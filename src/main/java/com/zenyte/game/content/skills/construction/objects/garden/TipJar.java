...
public final class TipJar implements ObjectInteraction {
    ...
    public void handleObjectAction(final Player player, final Construction construction, final RoomReference reference, final WorldObject object, final int optionId, final String option) {
        if (optionId == 4) {
            ...
            player.getDialogueManager().start(new OptionDialogue(player, "Tip jar: " + jar.getAmount() + " coin" + (jar.getAmount() == 1 ? "" : "s"),
                    new String[]{
                            jar.isDisplayingNotifications() ? "Disable notifications." : "Enable notifications.",
                            jar.isBankCoinsOnLogout() ? "Disable banking coins on logout." : "Enable banking coins on logout.",
                            "Withdraw money.",
                            "Cancel."
                    },
                    new Runnable[]{
                            () -> jar.setDisplayingNotifications(!jar.isDisplayingNotifications()),
                            () -> jar.setBankCoinsOnLogout(!jar.isBankCoinsOnLogout()),
                            () -> jar.withdraw(),
                            null
                    }));
        } else if (optionId == 1) {
