...
public enum TreasureChestData {
    ...
    public static final ItemContainer[] CONTAINERS = new ItemContainer[]{new ItemContainer(40, false), new ItemContainer(40, false), new ItemContainer(40, false), new ItemContainer(40, false), new ItemContainer(40, false), new ItemContainer(40, false), new ItemContainer(13, false)};
    public static final Map<Integer, TreasureChestData> MAP = new HashMap<>(VALUES.length * 5);
    public static final Map<Integer, TreasureChestData> DISPLAY_MAP = new HashMap<>(VALUES.length);

    static {
        for (int i = 0; i <= ELEGANT_CLOTHES_PINK.ordinal(); i++) CONTAINERS[0].add(new Item(VALUES[i].displayItem));
        CONTAINERS[0].add(new Item(MORE.displayItem));
        int index = 39;
        for (int i = 1; i < CONTAINERS.length - 1; i++) {
            for (int x = 0; x < 40; x++) {
                if (x == 0) CONTAINERS[i].add(new Item(BACK.displayItem));
                else if (x == 39) CONTAINERS[i].add(new Item(MORE.displayItem));
                else CONTAINERS[i].add(new Item(VALUES[index++].displayItem));
            }
        }
