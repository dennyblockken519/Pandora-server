...
public final class Staircase implements ObjectInteraction {
    ...
    public Object[] getObjects() {
        return new Object[]{13497, 13498, 13499, 13500, 13501, 13502, 13503, 13505};
    }
    ...
    public void handleObjectAction(Player player, Construction construction, RoomReference reference, WorldObject object, int optionId, String option) {
        if (optionId == 1) {
            ...
            if (ref == null) {
                boolean upstairs = player.getPlane() == 1 || reference.getRoom() == Room.SKILL_HALL || reference.getRoom() == Room.QUEST_HALL;
            ...
            final RoomReference ref = player.getConstruction().getReference(tile);
            if (ref == null)
                player.getDialogueManager().start(new ClimbEmptyStaircaseD(player, reference, reference.getRoom() == Room.SKILL_HALL || reference.getRoom() == Room.QUEST_HALL, object));
            else
                player.setLocation(tile);
        } else if (optionId == 4) {
            if (player.getPlane() == 2) {
