...
public enum SpellbookTeleport implements TeleportSpell, Teleport {
    ...
    LUMBRIDGE_HOME_TELEPORT(NORMAL, HOME_TELEPORT, 0, new Location(3222, 3218, 0), true),
    ...
    SEERS_TELEPORT(NORMAL, REGULAR_TELEPORT, 55.5, new Location(2725, 3485, 0)) {
        ...
        public void teleport(final Player player) {
            try {
                if (!DiaryReward.KANDARIN_HEADGEAR3.eligibleFor(player)) {
                ...
                Magic.logger.error(Strings.EMPTY, e);
            }
        }
    },
    ...
    APE_ATOLL_TELEPORT_REG(NORMAL, REGULAR_TELEPORT, 74, new Location(2796, 2799, 1)) {
        @Override
        public Item[] getRunes() {
            return new Item[]{new Item(Magic.LAW_RUNE, 2), new Item(Magic.WATER_RUNE, 2), new Item(Magic.FIRE_RUNE, 2), new Item(1963, 1)};
        }
    ...
    ANCIENT_ZENYTE_HOME_TELEPORT(ANCIENT, HOME_TELEPORT, 0, new Location(3087, 3490, 0), true) {
        @Override
        public String toString() {
            return GameConstants.SERVER_NAME + " home teleport";
        }
    },
    ...
    LUNAR_ZENYTE_HOME_TELEPORT(LUNAR, HOME_TELEPORT, 0, new Location(3087, 3490, 0), true) {
        @Override
        public String toString() {
            return GameConstants.SERVER_NAME + " home teleport";
        }
    },
    LUNAR_HOME_TELEPORT(LUNAR, HOME_TELEPORT, 0, new Location(2086, 3913, 0), true),
    ...
    ARCEUUS_ZENYTE_HOME_TELEPORT(ARCEUUS, HOME_TELEPORT, 0, new Location(3087, 3490, 0), true) {
        @Override
        public String toString() {
            return GameConstants.SERVER_NAME + " home teleport";
        }
    },
    ARCEUUS_HOME_TELEPORT(ARCEUUS, HOME_TELEPORT, 0, new Location(1699, 3879, 0), true),
    ...
    SpellbookTeleport(final Spellbook spellbook, final TeleportType type, final double experience, final Location destination, final boolean combatRestricted) {
        this(spellbook, type, experience, destination, 20, combatRestricted);
    }

    SpellbookTeleport(final Spellbook spellbook, final TeleportType type, final double experience, final Location destination, final int wildernessLevel, final boolean combatRestricted) {
    ...
    public void execute(final Player player, final int optionId, final String option) {
        ...
        if (this == ZENYTE_HOME_TELEPORT || this == ANCIENT_ZENYTE_HOME_TELEPORT || this == LUNAR_ZENYTE_HOME_TELEPORT || this == ARCEUUS_ZENYTE_HOME_TELEPORT) {
            ...
            if (option.equalsIgnoreCase("Lunar Isle")) {
                ...
                if (player.getMemberRank().eligibleTo(MemberRank.DRAGONSTONE_MEMBER)) {
                    ZENYTE_HOME_TELEPORT_INSTANT.teleport(player);
                    return;
                }
            }
            if (optionId == 10) {
                ...
                player.getDialogueManager().start(new Dialogue(player) {
                    ...
                    public void buildDialogue() {
                        ...
                        plain(20, "You have activated the RuneFest 2014 home teleport animation. In areas where this animation cannot play, the default animation will be used instead.");
                    }
                });
                return;
            }
        ...
        if (option.equals("Configure")) {
            switch (this) {
                case VARROCK_TELEPORT:
                    ...
                    player.sendMessage("Varrock teleport order has been reversed; left-click teleport now teleports you to: " + (player.getBooleanSetting(Setting.VARROCK_TELEPORT_CONFIGURATION) ? "Grand Exchange" : "Varrock") + ".");
                    break;
                case CAMELOT_TELEPORT:
                    player.getSettings().toggleSetting(Setting.CAMELOT_TELEPORT_CONFIGURATION);
                    player.sendMessage("Camelot teleport order has been reversed; left-click teleport now teleports you to: " + (player.getBooleanSetting(Setting.CAMELOT_TELEPORT_CONFIGURATION) ? "Seers' village" : "Camelot") + ".");
                    break;
                case WATCHTOWER_TELEPORT:
                    player.getSettings().toggleSetting(Setting.WATCHTOWER_TELEPORT_CONFIGURATION);
                    player.sendMessage("Watchtower teleport order has been reversed; left-click teleport now teleports you to: " + (player.getBooleanSetting(Setting.WATCHTOWER_TELEPORT_CONFIGURATION) ? "Yanille" : "Watchtower") + ".");
                    break;
                default:
                    break;
            }
        ...
        switch (this) {
            case TELEPORT_TO_BOUNTY_TARGET:
                setting = Setting.BOUNTY_TARGET_TELEPORT_WARNING;
                break;
            case DAREEYAK_TELEPORT:
                setting = Setting.DAREEYAK_TELEPORT_WARNING;
                break;
            case CARRALLANGAR_TELEPORT:
                setting = Setting.CARRALLANGAR_TELEPORT_WARNING;
                break;
            case ANNAKARL_TELEPORT:
                setting = Setting.ANNAKARL_TELEPORT_WARNING;
                break;
            case GHORROCK_TELEPORT:
                setting = Setting.GHORROCK_TELEPORT_WARNING;
                break;
            default:
                setting = null;
                break;
        ...
        if (setting != null) {
            ...
            if (optionId == 1 && player.getBooleanSetting(setting)) {
                warn(player);
                return;
            }
        }
