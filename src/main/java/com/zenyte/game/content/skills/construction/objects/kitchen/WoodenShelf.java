...
public final class WoodenShelf implements ObjectInteraction {
    ...
    public Object[] getObjects() {
        return new Object[]{13545, 13546, 13554};
    }
    ...
    public void handleObjectAction(final Player player, final Construction construction, final RoomReference reference, final WorldObject object, final int optionId, final String option) {
        ...
        player.getDialogueManager().start(new OptionDialogue(player, "What would you like to take?",
                new String[]{
                        "A kettle.",
                        "A teapot.",
                        objId == 13554 ? "A porcelain cup." : "A clay cup.",
                        objId == 13545 ? null : "An empty beer glass.",
                        objId == 13554 ? "A cake tin." : null
                }, new Runnable[]{
                () -> addItem(player, 0, objId),
                () -> addItem(player, 1, objId),
                () -> addItem(player, 2, objId),
                () -> addItem(player, 3, objId),
                () -> addItem(player, 4, objId)
        }));
    }
