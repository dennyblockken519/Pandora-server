        ...
        if (!regeneratingFruit) {
            ...
            if (newState == GROWN && newState != oldState) {
                getCompostFlag().ifPresent(flag -> {
                    final int extra = (player.getMemberRank().eligibleTo(MemberRank.RUBY_MEMBER) && this.getPatch().getType() == PatchType.HERB_PATCH) || getPatch().getType() == PatchType.BUSH_PATCH ? 1 : 0;
                    if (flag == COMPOST) {
                        setLives(extra + 4);
